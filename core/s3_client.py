...
from core.client_identity import HTTP_USER_AGENT  # noqa: E402 — after log= for clean attribution
...
class S3Session:
    ...
    def __init__(
        ...
        if boto3 is None:
            raise ImportError("S3 support requires boto3. Install with: pip install axross[s3]")
        ...
        if proxy_host and proxy_type in {"socks4", "socks5"}:
            log.warning(
                "S3 proxy configuration uses %s, but botocore does not "
                "support SOCKS proxy URLs reliably. Use HTTP CONNECT or a "
                "local SOCKS-to-HTTP relay.",
                proxy_type,
            )
            raise OSError(
                "S3 SOCKS proxy support is not available with botocore. "
                "Use proxy_type='http' for HTTP CONNECT or a local "
                "SOCKS-to-HTTP relay."
            )
        ...
        if access_key and secret_key:
            ...
            kwargs["config"] = base_config.merge(BotoConfig(signature_version=UNSIGNED))
        else:
            kwargs["config"] = base_config
        if endpoint:
        ...
        log.info(
            "S3 connected: %s (region=%s, endpoint=%s, anonymous=%s)",
            bucket,
            self._region,
            endpoint or "default",
            self._anonymous,
    ...
    def open_version_read(self, path: str, version_id: str):
        ...
        try:
            ...
            raise OSError(f"S3 get_object {path}@{version_id}: {exc}") from exc
    ...
    def presign(
        ...
        *,
        ...
        log.info(
            "S3 presign: bucket=%s key=%s method=%s expires=%ds",
            self._bucket,
            self._to_key(path),
            method,
            int(expires),
    ...
    def checksum(self, path: str, algorithm: str = "sha256") -> str:
        ...
        """
        try:
            resp = self._s3.head_object(Bucket=self._bucket, Key=self._to_key(path))
        except Exception as exc:
