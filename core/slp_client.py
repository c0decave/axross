    ...
    def __init__(
        ...
        **_ignored,
        ...
        warn_unsupported_proxy(
            ...
            "SLP defaults to UDP and axross does not implement SOCKS UDP-ASSOCIATE.",
        )
        self._host = host
        self._port = int(port)
        ...
        log.info(
            "SLP session ready: %s:%d (scope=%s, %d types)",
            host,
            self._port,
            scope,
            len(self._types_cache or []),
        )
        ...
        for svc_type in self._fetch_types():
            ...
            for url, _lifetime in urls_with_lifetime:
                ...
                try:
                    ...
                    log.debug(
                        "SLP attrs fetch failed for %s: %s",
                        redact_url(url),
                        exc,
                    )
                out.append(
