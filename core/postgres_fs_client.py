...
class PostgresFsSession(DbFsBackend):
    ...
    def __init__(
        ...
        **_ignored,
        ...
        warn_unsupported_proxy(
            ...
            "psycopg connections are not currently wrapped by core.proxy.",
        )
        if psycopg is None:
            raise ImportError(
                "PostgreSQL backend requires psycopg. Install with: pip install axross[postgres]",
            )
        if url:
            parts = urlsplit(url)
        ...
        self._dsn = f"host={host} port={port} user={username} password={password} dbname={database}"
    ...
    def close(self) -> None:
        log.info("PostgreSQL-FS disconnecting: %s", self._display)
        try:
            self._conn.close()
    ...
    def tables(self, schema: str = "public") -> list[str]:
        ...
        if schema == "*":
            rows = self.query(
                "SELECT schemaname || '.' || tablename AS qname FROM pg_tables ORDER BY 1"
            )
