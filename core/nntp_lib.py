...
# Per-line cap. The multi-line cap only triggers after ``readline()``
# returns; a server that withholds newlines must still be bounded.
    ...
    def readline(self) -> bytes:
        while b"\n" not in self._buf:
            ...
            if len(self._buf) > MAX_LINE_BYTES:
                raise NntpError(f"NNTP line exceeds {MAX_LINE_BYTES} byte cap")
...
class NntpClient:
    ...
    def post(self, raw_article: bytes) -> None:
        ...
        # PATTERN-001: bytes.splitlines() splits on \v/\f/\x1c-\x1e plus
        # bare \r/\n. Use LF-only split + strip CRLF artefact so an
        # article carrying \v in content (e.g. a binary attachment header)
        # cannot fan out into multiple dot-stuffed wire-lines.
        out = bytearray()
        for line in raw_article.split(b"\n"):
