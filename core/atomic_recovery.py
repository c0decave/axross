...
_ORPHAN_PATTERN = re.compile(r"^\.(?:tmp|axross-atomic)-[0-9a-f]{12}\.tmp$")
...
def sweep_orphans(
    ...
    *prefetched_entries* is the optional list returned by an earlier
    ...
    for entry in entries:
        ...
        if mtime > cutoff:
            log.debug(
                "atomic_recovery: skipping %s — too young (mtime %.0f > cutoff %.0f)",
                name,
                mtime,
                cutoff,
            )
        ...
        try:
            ...
            log.warning(
                "atomic_recovery: join(%s, %s) failed: %s",
                root,
                name,
                exc,
            )
        ...
        try:
            ...
            log.warning(
                "atomic_recovery: remove(%s) failed: %s",
                full,
                exc,
            )
    return removed
