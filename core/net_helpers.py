...
def tcp_banner(
    host: str, port: int, *, timeout: float = 3.0, max_bytes: int = 4096, send: bytes | None = None
) -> bytes:
...
def ping(host: str, *, port: int = 80, timeout: float = 3.0, count: int = 1) -> list[PingResult]:
...
def port_scan(
    host: str, ports: Iterable[int], *, timeout: float = 1.0, concurrency: int = 64
) -> list[int]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as pool:
        futures = {pool.submit(port_open, host, p, timeout=timeout): p for p in port_list}
...
def dns_records(
    name: str, rtype: str = "A", *, resolver: str | None = None, timeout: float = 3.0
) -> list[str]:
...
def dns_reverse(ip: str, *, resolver: str | None = None, timeout: float = 3.0) -> list[str]:
    ...
    *,
    ...
    if parsed.scheme == "https":
        try:
            ...
            log.debug(
                "tls_cert side-fetch failed for %s: %s",
                redact_url(resp.url),
                exc,
            )
    return HttpProbe(
...
def _walk(
    ...
    *,
    ...
    for item in items:
        try:
            full = safe_join(backend, path, item.name)
        except ValueError as exc:
            log.warning(
                "find_files: skipping unsafe entry name at %s: %s",
                path,
                exc,
            )
            continue
...
def archive_inspect(path: str, *, max_entries: int = 100_000) -> list[ArchiveEntry]:
    ...
    * 7z — ``py7zr`` (axross[archive] extra)
    ...
    if (
        ...
        or head.startswith(b"\xfd7zXZ")
    ):
        # gzip / bzip2 / xz wrap a tar in practice.
        return _archive_inspect_tar(path, max_entries)
    raise OSError(f"archive_inspect: cannot identify {path!r} (unknown magic / extension)")
...
def _archive_inspect_7z(path: str, max_entries: int) -> list[ArchiveEntry]:
    try:
        ...
        raise OSError(
            "archive_inspect 7z support requires py7zr — install with `pip install axross[archive]`"
        ) from exc
    out: list[ArchiveEntry] = []
    try:
...
def time_skew(
    ...
    * ``"tls"``  — TLS 1.2 ServerHello carries gmt_unix_time in
    ...
    raise ValueError(f"time_skew source must be ntp/http/tls, got {source!r}")
...
def _time_skew_ntp(host: str, *, port: int, timeout: float) -> TimeSkew:
    ...
    start = time.monotonic()
    try:
        resp = client.request(host, port=int(port), timeout=float(timeout), version=3)
...
def _time_skew_http(
    host: str, *, port: int | None, verify_tls: bool = False, timeout: float
) -> TimeSkew:
    ...
    for scheme in schemes:
        ...
        try:
            # verify=verify_tls is False by default — see time_skew()
            # docstring for the threat-model rationale (we only consume
            # the Date: header, never the body; MITM can corrupt the
            # skew reading, never extract secrets).
            resp = requests.head(
                url, timeout=float(timeout), allow_redirects=False, verify=verify_tls
            )
        except Exception as exc:  # noqa: BLE001
            last_exc = exc
            continue
...
def _normalise_mac(mac: str) -> str:
    ...
    if len(cleaned) != 12 or not all(c in "0123456789abcdefABCDEF" for c in cleaned):
        raise ValueError(f"mac_lookup: {mac!r} is not a 12-hex-digit MAC address")
...
def diff_files(
    b1, p1: str, b2, p2: str, *, max_bytes: int = 10 * 1024 * 1024, n_context: int = 3
) -> list[str]:
