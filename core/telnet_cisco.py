...
def _safe_show_subcmd(name: str) -> str:
    ...
    if not re.fullmatch(r"[A-Za-z0-9 _\-]+", cleaned):
        raise OSError(f"Cisco-show name {name!r} contains characters outside [A-Za-z0-9 _-]")
...
class CiscoTelnetSession:
    ...
    def _login(self) -> None:
        ...
        if _LOGIN_RE.search(text):
            ...
            blob, _m = self._transport.read_until_re(
                re.compile(_LOGIN_RE.pattern + "|" + _PASSWORD_RE.pattern, re.IGNORECASE),
    ...
    def _enter_enable_mode(self) -> None:
        ...
        if _PRIV_PROMPT_RE.search(text):
            ...
            raise OSError(
                "Cisco-Telnet: enable-password rejected (prompt did not flip to privileged mode)"
            )
    ...
    def save_running_config(self) -> str:
        ...
        log.info(
            "Cisco WRITE MEMORY on %s@%s — persisting running-config to NVRAM",
            self._username or "(line)",
            self._host,
        )
    ...
    def clear_counters(self, interface: str | None = None) -> str:
        ...
        if interface is not None:
            if "\r" in interface or "\n" in interface:
                raise ValueError("clear_counters interface must not contain CR/LF")
            log.info(
                "Cisco CLEAR COUNTERS on %s@%s interface=%s",
                self._username or "(line)",
                self._host,
                interface,
            )
        ...
        log.info(
            "Cisco CLEAR COUNTERS on %s@%s (all interfaces)",
            self._username or "(line)",
            self._host,
        )
    ...
    def _send_show(self, command: str) -> str:
        ...
        if len(blob) > MAX_SHOW_BYTES:
            raise OSError(f"Cisco-show response exceeds {MAX_SHOW_BYTES} byte cap")
