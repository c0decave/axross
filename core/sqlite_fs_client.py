...
class SqliteFsSession(DbFsBackend):
    ...
    def __init__(self, url: str = "", **_ignored):
        ...
        warn_unsupported_proxy(
            ...
            "SQLite is local file IO, not a network transport.",
        )
    ...
    def close(self) -> None:
        log.info("SQLite-FS closing: %s", self._db_path)
        try:
            self._conn.close()
    ...
    def _db_get(self, path: str) -> dict | None:
        with self._lock:
            cur = self._conn.execute(
                "SELECT * FROM axross_files WHERE path = ?",
                (path,),
            )
    ...
    def _db_delete_subtree(self, path: str) -> None:
        ...
        with self._lock:
            self._conn.execute(
                "DELETE FROM axross_files WHERE path = ? OR path LIKE ? || '/%' ESCAPE '\\'",
                (path, escaped),
            )
