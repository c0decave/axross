...
def browse(backend, path: str) -> list[SnapshotEntry]:
    ...
    try:
        ...
        log.warning("snapshot_browser.browse(%s, %s) failed: %s", type(backend).__name__, path, exc)
        return []
    return [SnapshotEntry(backend=backend, path=path, version=v) for v in versions]
