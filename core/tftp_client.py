...
def _validate_filename(name: str) -> str:
    ...
    if not _FILENAME_RE.match(cleaned):
        raise OSError(
            f"TFTP filename {name!r} contains characters outside the [A-Za-z0-9._/-] safe set",
        )
    return cleaned
...
class TftpSession:
    ...
    def __init__(
        ...
        warn_unsupported_proxy(
            ...
            proxy_type,
            proxy_host,
            "TFTP is UDP; SOCKS/HTTP CONNECT support is not implemented.",
        )
        if not TFTPY_AVAILABLE:
            raise ImportError(
                "TFTP support requires tftpy. Install with: pip install axross[tftp]",
            )
        self._host = host
        self._port = int(port)
    ...
    def exists(self, path: str) -> bool:
        ...
        if path == "/" or self.is_dir(path):
            return True
        name = _validate_filename(path)
        if self._filelist_enabled:
            return name in self._filelist
        # TFTP has no metadata operation. A read probe is the only
        # reliable existence check; the normal read size cap still
        # applies so this cannot fetch an unbounded file.
        try:
            with self.open_read(path):
                return True
        except OSError:
            return False
    ...
    def open_read(self, path: str, mode: str = "rb") -> IO:
        ...
        def _packethook(pkt):
            ...
            if bytes_so_far[0] > cap:
                raise OSError(
                    f"TFTP read of {name!r} exceeded {cap} byte cap (at {bytes_so_far[0]} bytes)",
                )
    ...
    def open_write(self, path: str, append: bool = False) -> IO:
        ...
        """
        if append:
            raise OSError("TFTP WRQ does not support append.")
        path = self.normalize(path)
        name = _validate_filename(path)
