...
def _parse_menu(raw: bytes) -> list[dict]:
    ...
    entries: list[dict] = []
    for line in split_lf_only_stream(text):
        if line == ".":
            break
        if not line:
            continue
    ...
    * ``"/foo/bar"`` → selector ``/foo/bar`` (most servers expect this
    ...
    def _menu_for_path(self, path: str) -> list[dict]:
        ...
        if path.count("/") > MAX_PATH_DEPTH:
            raise OSError(f"Gopher path nesting exceeds {MAX_PATH_DEPTH} levels: {path!r}")
    ...
    def recursive_fetch(
        self, path: str = "/", *, max_files: int = 1000, max_depth: int = 10, on_visit=None
    ) -> dict[str, bytes]:
