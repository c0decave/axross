...
class PjlSession:
    ...
    def __init__(
        ...
        if not _VOLUME_RE.match(cleaned_volume):
            raise OSError(f"PJL volume {volume!r} is outside the safe set [0-9A-Za-z][:]")
    ...
    def _to_pjl_path(self, path: str) -> str:
        ...
        if not _PATH_RE.match(path):
            raise OSError(f"PJL path contains characters outside [0-9A-Za-z._/:-]: {path!r}")
        if path == "/":
    ...
    def _parse_fsdirlist(blob: bytes) -> list[FileItem]:
        ...
        for raw in split_lf_only_stream(text):
            line = raw.strip()
    ...
    def _upload(self, path: str, data: bytes) -> None:
        ...
        sock = _open_socket(self._host, self._port, self._proxy, self._timeout)
        try:
            header = (f'@PJL FSDOWNLOAD FORMAT:BINARY SIZE={size} NAME="{target}"\n').encode()
