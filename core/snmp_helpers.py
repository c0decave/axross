...
def _build_auth(
    ...
    if user:
        ...
        return UsmUserData(
            ...
            auth_key,
            priv_key,
            authProtocol=proto_map_a.get((auth_proto or "sha").lower(), usmHMACSHAAuthProtocol),
            privProtocol=proto_map_p.get((priv_proto or "aes").lower(), usmAesCfb128Protocol),
        )
...
async def _snmp_get_async(
    ...
    *,
    ...
    auth = _build_auth(community, user, auth_key, priv_key, auth_proto, priv_proto)
    ...
    if error_status:
        raise OSError(
            f"SNMP GET {host}:{port} {oid}: {error_status.prettyPrint()} (index {error_index})"
        )
    if not var_binds:
...
async def _snmp_walk_async(
    ...
    *,
    ...
    async for error_indication, error_status, error_index, var_binds in walk_cmd(
        ...
        if error_indication:
            raise OSError(f"SNMP WALK {host}:{port} {base_oid}: {error_indication}")
        if error_status:
            raise OSError(
...
def snmp_set(
    ...
    *,
    ...
    """
    if value_type not in _SET_VALUE_TYPES:
        expected = " / ".join(sorted(_SET_VALUE_TYPES))
        raise ValueError(f"snmp_set: unknown value_type {value_type!r}; expected one of {expected}")
    log.info(
        "SNMP SET %s:%d %s = %r (type=%s)",
        host,
        port,
        oid,
        value,
...
async def _snmp_set_async(host, oid, value, *, value_type, community, port, timeout, retries):
    ...
    if error_status:
        raise OSError(
            f"SNMP SET {host}:{port} {oid}: {error_status.prettyPrint()} (index {error_index})"
        )
    return _to_var(var_binds[0])
