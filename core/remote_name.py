...
def validate_remote_name(
    ...
    *name* is expected to be a **single path component** — pass
    ...
    if len(encoded) > max_bytes:
        raise RemoteNameError(f"name exceeds {max_bytes} bytes (utf-8, got {len(encoded)})")
    if "\x00" in name:
    ...
    if allow_separators:
        ...
        for part in parts:
            if part in (".", ".."):
                raise RemoteNameError(f"path contains traversal component: {part!r}")
        return
...
def is_safe_remote_name(
    ...
    *,
    ...
    if not isinstance(name, str):
        return False
    try:
        validate_remote_name(name, max_bytes=max_bytes, allow_separators=allow_separators)
