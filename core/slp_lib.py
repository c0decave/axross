...
def _build_header(
    function: int, body_len: int, xid: int | None = None, lang_tag: bytes = b"en"
) -> bytes:
...
def build_srv_req(svc_type: bytes, scope: bytes = b"DEFAULT", predicate: bytes = b"") -> bytes:
...
def build_attr_req(url: bytes, scope: bytes = b"DEFAULT", tags: bytes = b"") -> bytes:
...
def _read_u16(buf: bytes, off: int) -> tuple[int, int]:
    if off + 2 > len(buf):
        raise ValueError(f"SLP u16 read at offset {off} would overrun buffer of length {len(buf)}")
    try:
...
def _read_str(buf: bytes, off: int) -> tuple[bytes, int]:
    ...
    if off + length > len(buf):
        raise ValueError(
            f"SLP string of length {length} at offset {off} overruns buffer of length {len(buf)}"
        )
...
def parse_srv_type_reply(blob: bytes) -> list[str]:
    ...
    if not lst:
        return []
    return [s.decode("utf-8", "replace").strip() for s in lst.split(b",") if s.strip()]
...
def query_udp(
    host: str, packet: bytes, port: int = DEFAULT_PORT, timeout: float = DEFAULT_TIMEOUT
) -> bytes:
    ...
    if host in ("239.255.255.253",) or host.startswith("224."):
        raise OSError("SLP backend refuses to query multicast (CVE-2023-29552 mitigation)")
