...
class RshSession:
    ...
    def __init__(
        ...
        **_ignored,
        ...
        warn_unsupported_proxy(
            ...
            "The system rsh binary owns the TCP connection.",
        )
        self._host = host
        self._port = int(port)
        ...
        if "axross-rsh-probe" not in out.decode("utf-8", "replace"):
            raise OSError(f"rsh probe to {host} returned unexpected payload: {out!r}")
    ...
    def _run_remote(self, command: str, *, stdin: bytes | None = None) -> bytes:
        ...
        if proc.returncode != 0:
            ...
            raise RshError(
                f"rsh '{command[:60]}...' on {self._host} exited {proc.returncode}: {err}"
            )
        return proc.stdout
    ...
    def list_dir(self, path: str) -> list[FileItem]:
        ...
        items: list[FileItem] = []
        for raw in split_lf_only_stream(out.decode("utf-8", "replace")):
            line = raw.strip()
            ...
            items.append(
                FileItem(
                    name=name,
                    is_dir=is_dir,
                    is_link=perms_text.startswith("l"),
                    size=size,
    ...
    def open_read(self, path: str) -> IO[bytes]:
        ...
        cmd = f"head -c {MAX_FILE_BYTES} -- {shlex.quote(path)} && true"
    ...
    def rename(self, src: str, dst: str) -> None:
        ...
        self._run_remote(f"mv -- {shlex.quote(src)} {shlex.quote(dst)}")

    def chmod(self, path: str, mode: int) -> None:
        path = _validate_path(self.normalize(path))
        self._run_remote(f"chmod {oct(mode)[2:]} -- {shlex.quote(path)}")
    ...
    def copy(self, src: str, dst: str) -> None:
        ...
        src = _validate_path(self.normalize(src))
        dst = _validate_path(self.normalize(dst))
        self._run_remote(f"cp -p -- {shlex.quote(src)} {shlex.quote(dst)}")
    ...
    def disk_usage(self, path: str) -> tuple[int, int, int]:
        ...
        lines = [ln for ln in split_lf_only_stream(out) if ln.strip()]
        if len(lines) < 2:
            return (0, 0, 0)
        parts = lines[-1].split()
        try:
