...
def parse_ssh_config(path: Path | None = None) -> list[SSHHostConfig]:
    ...
    try:
        if path.stat().st_size > MAX_SSH_CONFIG_BYTES:
            log.warning(
                "SSH config %s exceeds %d bytes; refusing to parse",
                path,
    ...
    for line in text.splitlines():
        ...
        if key == "host":
            aliases = [alias for alias in value.split() if "*" not in alias and "?" not in alias]
