...
def list_devices(mounter: str | None = None) -> list[MtpDevice]:
    ...
    try:
        ...
        raise OSError(
            f"{chosen} -l timed out after {LIST_DEVICES_TIMEOUT_SECONDS}s",
        ) from exc
...
def _parse_device_listing(mounter: str, raw: str) -> list[MtpDevice]:
    out: list[MtpDevice] = []
    if mounter == "jmtpfs":
        for line in split_lf_only_stream(raw):
            line = line.strip()
...
class MtpSession(LocalFS):
    ...
    def __init__(
        ...
        *,
        ...
        warn_unsupported_proxy(
            ...
            proxy_type,
            proxy_host,
            "MTP is USB/FUSE based here and has no TCP transport to tunnel.",
        )
        super().__init__()
        ...
        if not _DEVICE_ID_SAFE.match(device_id or ""):
            raise ValueError(
                f"MTP device_id must match {_DEVICE_ID_SAFE.pattern!r}; got {device_id!r}",
            )
        ...
        if self._mounter is None:
            raise FileNotFoundError(
                f"no MTP FUSE mounter on PATH (tried {', '.join(_KNOWN_MOUNTERS)})",
            )
    ...
    def _build_mount_command(self) -> list[str]:
        ...
        raise OSError(
            f"unsupported mounter {self._mounter!r} — add a case in _build_mount_command",
        )
    ...
    def _unmount(self, timeout: float = 5.0) -> None:
        ...
        # do is warn.
        for cmd in (("fusermount", "-u", self._mount_dir), ("umount", self._mount_dir)):
            try:
                result = subprocess.run(
                    cmd,
                    capture_output=True,
