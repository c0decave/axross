...
def decrypt_bytes(blob: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise InvalidCiphertext(f"key derivation failed: {exc}") from exc
    try:
...
def write_encrypted(backend, path: str, data: bytes, passphrase: str) -> str:
    ...
    atomic_write(backend, final, blob)
    return final
...
def read_encrypted(backend, path: str, passphrase: str) -> bytes:
    ...
    if len(raw) > MAX_ENCRYPTED_SIZE:
        raise InvalidCiphertext(
            f"encrypted blob at {path} exceeds {MAX_ENCRYPTED_SIZE // (1024 * 1024)} MiB cap"
        )
...
def encrypt_stream(
    ...
    *,
    ...
    if chunk_size <= 0 or chunk_size > STREAM_MAX_CHUNK_BYTES:
        raise ValueError(
            f"chunk_size must be in (0, {STREAM_MAX_CHUNK_BYTES}]; got {chunk_size}",
        )
    salt = secrets.token_bytes(SALT_LEN)
    key = _derive_key(passphrase, salt)
    aead = AESGCM(key)
    ...
    while pending:
        ...
        nonce = secrets.token_bytes(NONCE_LEN)
        ad = MAGIC_V2 + index.to_bytes(8, "little") + bytes([is_final])
        ct = aead.encrypt(nonce, pending, associated_data=ad)
        writer.write(
            len(ct).to_bytes(4, "little") + bytes([is_final]) + nonce + ct,
        )
        pending = lookahead
        index += 1
    ...
    if index == 0:
        ...
        writer.write(
            len(ct).to_bytes(4, "little") + b"\x01" + nonce + ct,
        )
...
def decrypt_stream(
    ...
    if chunk_size <= 0 or chunk_size > STREAM_MAX_CHUNK_BYTES:
        raise InvalidCiphertext(
            f"header declares chunk_size {chunk_size} outside (0, {STREAM_MAX_CHUNK_BYTES}]",
        )
    try:
        key = _derive_key(passphrase, salt)
    except Exception as exc:
    ...
    while True:
        ...
        if ct_len > max_ct_len:
            raise InvalidCiphertext(
                f"chunk {index} ct_len {ct_len} exceeds max_ct_len {max_ct_len}",
            )
        ...
        try:
            ...
            raise InvalidCiphertext(
                f"chunk {index} authentication failed — wrong passphrase or tampered blob",
            ) from exc
        writer.write(pt)
        if is_final:
