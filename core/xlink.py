...
def _decode(raw: bytes) -> CrossProtocolLink:
    ...
    if data.get("schema") != SCHEMA_TAG:
        raise ValueError(
            f"xlink schema mismatch: expected {SCHEMA_TAG!r}, got {data.get('schema')!r}"
        )
    version = int(data.get("version") or 0)
    if version < 1 or version > SCHEMA_VERSION:
        raise ValueError(
            f"xlink version {version} is not supported by this build (max {SCHEMA_VERSION})"
        )
...
def is_xlink(backend, path: str) -> bool:
    ...
    if len(raw) > MAX_LINK_SIZE:
        log.warning("is_xlink: %s exceeds %d bytes — refusing to parse", path, MAX_LINK_SIZE)
        return False
    try:
        _decode(raw)
    except ValueError as exc:
...
def create_xlink(backend, path: str, target_url: str, display_name: str = "") -> str:
    ...
    atomic_write(backend, final, data)
    return final
...
def read_xlink(backend, path: str) -> CrossProtocolLink:
    ...
    if isinstance(raw, str):
        raw = raw.encode("utf-8", errors="replace")
    if len(raw) > MAX_LINK_SIZE:
        raise ValueError(f"xlink payload at {path} exceeds {MAX_LINK_SIZE} bytes")
    return _decode(raw)
