...
class NfsSession:
    ...
    def __init__(
        ...
        warn_unsupported_proxy(
            ...
            proxy_type,
            proxy_host,
            "NFS is mounted by the kernel; user-space proxy settings cannot apply.",
        )
        self._host = host
    ...
    def _connect(self) -> None:
        ...
        if self._version >= 4:
            ...
            opts = f"vers={self._version},port={self._port},mountport={self._port},nolock,tcp"
    ...
    def exports_list(host: str, *, timeout: float = 5.0) -> list[dict]:
        ...
        if "\r" in host or "\n" in host or " " in host:
            raise ValueError("exports_list host must not contain CR/LF/space")
