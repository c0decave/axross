...
Server has:
  ...
  - ~/data/readme.txt
  - ~/data/bigfile.bin (5MB)
  - ~/data/subdir/nested.txt
  - ~/data/link_to_readme (symlink)
"""
...
def test_ssh_wrong_password():
    ...
    session = SSHSession(profile)
    try:
        session.connect(password="wrong" + "pass")
        session.disconnect()
...
def test_symlinks():
    ...
    try:
        ...
        assert len(link_items) == 1, "link_to_readme not found in listing"
        link = link_items[0]
...
def test_proxy_config():
    ...
    try:
        ...
        raise AssertionError("half-configured proxy must fail closed")
