...
# user 'axuser' with ~/.rhosts authorising access from the
# test runner.
#
...
RUN useradd -m -s /bin/bash axuser && \
    echo "+ +" > ~/.rhosts && \
    chmod 600 ~/.rhosts && \
    chown axuser:axuser ~/.rhosts && \
    install -d -m 0755 -o axuser -g axuser ~/data && \
    echo "hello from rshd" > ~/data/seed.txt && \
    chown axuser:axuser ~/data/seed.txt
