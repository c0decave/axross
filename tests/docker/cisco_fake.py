...
SHOWS: dict[str, str] = {
    ...
    "startup-config": (f"Using 1024 out of 524288 bytes\r\n!\r\nhostname {HOSTNAME}\r\nend\r\n"),
    "interfaces": (
    ...
    "clock": ("*04:25:17.123 UTC Sat Apr 26 2026\r\n"),
    "logging": (
    ...
    def _serve(self, conn: socket.socket) -> None:
        ...
        while True:
            ...
            if low.startswith("show "):
                ...
                # suite cares about.
                sub = cmd[5:].strip()
                if not privileged and sub.lower() in ("running-config", "startup-config"):
            ...
            if low == "clear counters" or low.startswith("clear counters "):
                ...
                _send(
                    conn, f'Clear "show interface" counters on {target} interface(s) [confirm]\r\n'
                )
                continue
