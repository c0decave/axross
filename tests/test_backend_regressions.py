...
class ProfileSecretRegressionTests(unittest.TestCase):
    ...
    def test_load_rejects_non_object_or_oversize_profiles_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            profiles = Path(tmpdir) / "profiles.json"
            with (
                mock.patch("core.profiles.CONFIG_DIR", Path(tmpdir)),
                mock.patch("core.profiles.PROFILES_FILE", profiles),
            ):
                profiles.write_text("[]", encoding="utf-8")
                with self.assertLogs("core.profiles", level="ERROR"):
                    self.assertEqual(ProfileManager().list_names(), [])

                profiles.write_text('{"ok":{"name":"ok"}}', encoding="utf-8")
                with (
                    mock.patch("core.profiles.MAX_PROFILES_FILE_BYTES", 4),
                    self.assertLogs("core.profiles", level="ERROR"),
                ):
                    self.assertEqual(ProfileManager().list_names(), [])
...
class ConnectionManagerRegressionTests(unittest.TestCase):
    def test_session_key_includes_backend_specific_connection_settings(self) -> None:
        ...
        self.assertNotEqual(
            manager._session_key(iscsi_auto), manager._session_key(iscsi_other_mount)
        )
...
class BackendRegistryRegressionTests(unittest.TestCase):
    ...
    def test_registered_backend_classes_implement_core_filebackend_methods(self) -> None:
        import importlib
        ...
        for info in backend_registry.all_backends():
            ...
            for method in sorted(required):
                if not hasattr(cls, method):
                    missing.append(f"{info.protocol_id}.{method}")
...
"""
    ...
    def test_parse_ssh_config_rejects_oversize_file(self) -> None:
        from core.ssh_config import parse_ssh_config

        p = Path(tempfile.mkstemp(suffix=".sshconfig")[1])
        try:
            p.write_text("Host large\n", encoding="utf-8")
            with (
                mock.patch("core.ssh_config.MAX_SSH_CONFIG_BYTES", 4),
                self.assertLogs("core.ssh_config", level="WARNING"),
            ):
                self.assertEqual(parse_ssh_config(p), [])
        finally:
            p.unlink(missing_ok=True)
    ...
    def test_parse_ssh_config_skips_wildcards_in_host(self) -> None:
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".sshconfig", delete=False) as h:
            h.write("Host *\n    User everyone\nHost real\n    User me\n")
            p = Path(h.name)
        try:
...
class BundledScriptsTests(unittest.TestCase):
    ...
    def test_bulk_rename_rejects_unsafe_regex_and_preflights_targets(self) -> None:
        ns = self._load_script("bulk_rename")
        with tempfile.TemporaryDirectory() as tmp:
            (Path(tmp) / "raw_a.txt").write_text("data")
            (Path(tmp) / "trim_a.txt").write_text("existing")
            from core import scripting as s

            with self.assertRaisesRegex(ValueError, "unsafe"):
                ns["bulk_rename"](s.localfs(), tmp, r"(a+)+$", "x")
            ...
            self.assertTrue((Path(tmp) / "raw_a.txt").exists())
            self.assertEqual((Path(tmp) / "trim_a.txt").read_text(), "existing")

    def test_find_secrets_catches_aws_key(self) -> None:
        ns = self._load_script("find_secrets")
        with tempfile.TemporaryDirectory() as tmp:
            (Path(tmp) / "config").write_bytes(b"AKIAIOSFODNN7EXAMPLE\nrandom\n")
            from core import scripting as s
    ...
    def test_redact_rejects_unsafe_regex_before_walking(self) -> None:
        ns = self._load_script("redact")
        with tempfile.TemporaryDirectory() as tmp:
            from core import scripting as s
    ...
    def test_bookmarks_export_round_trip(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch.dict(os.environ, {"HOME": tmp}):
                ...
                s.add_bookmark(name="logs", path="/var/log", backend_name="Local")
                s.add_bookmark(name="etc", path="/etc", backend_name="Local")
    ...
    def test_gopher_archive_against_inproc_server(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertTrue((Path(tmp) / "mirror" / "readme.txt").exists())
        finally:
            stop()
    ...
    def test_hash_audit_detects_mismatch_and_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            wrong_hash = "0" * 64
            manifest = f"{ok_hash}\tok.txt\n{wrong_hash}\tbad.txt\n{wrong_hash}\tabsent.txt\n"
...
class ScriptingReferenceQualityTests(unittest.TestCase):
    ...
    def test_every_public_function_has_a_meaningful_docstring(self) -> None:
        ...
        self.assertFalse(
            too_short,
            f"docstrings shorter than {self.MIN_DOCSTRING_CHARS} chars: {too_short}",
        )
    ...
    def test_docs_returns_complete_markdown(self) -> None:
        ...
        for header in ("REPL slash-commands", "Bundled example scripts", "FileBackend` protocol"):
...
class ScriptingExpansionTests(unittest.TestCase):
    ...
    def test_imap_search_dispatches_imap_and_exchange_shapes(self) -> None:
        from core import scripting as s

        class _ImapLike:
            def search(self, criteria="ALL", *, mailbox="INBOX"):
                return [criteria, mailbox]

        class _ExchangeLike:
            def search(self, query, *, folder=None, limit=100):
                return [{"query": query, "folder": folder, "limit": limit}]

        self.assertEqual(
            s.imap_search(_ImapLike(), "UNSEEN", mailbox="Archive"),
            ["UNSEEN", "Archive"],
        )
        self.assertEqual(
            s.imap_search(_ExchangeLike(), "invoice", mailbox="INBOX"),
            [{"query": "invoice", "folder": None, "limit": 100}],
        )
        self.assertEqual(
            s.imap_search(_ExchangeLike(), "invoice", mailbox="/sent items"),
            [{"query": "invoice", "folder": "/sent items", "limit": 100}],
        )
...
class ScriptDirectoryTests(unittest.TestCase):
    ...
    def test_run_script_blocked_in_paranoid_mode(self) -> None:
        from core import scripting as s
        from core import security_mode

        s.save_script("blocked", "result = 42\n")
        old = security_mode.current_policy().name
        try:
            security_mode.set_policy("paranoid")
            with self.assertRaises(PermissionError):
                s.run_script("blocked")
        finally:
            security_mode.set_policy(old)
    ...
    def test_script_storage_does_not_follow_symlinks(self) -> None:
        ...
        script_dir = Path(s.script_dir())
        outside = Path(self.tmp_home) / "outside-secret.txt"
        outside.write_text("secret", encoding="utf-8")
        link = script_dir / "leak.py"
        os.symlink(outside, link)

        self.assertNotIn("leak", s.list_scripts())
        with self.assertRaises(OSError):
            s.load_script("leak")

        s.save_script("leak", "result = 1\n")
        self.assertEqual(outside.read_text(encoding="utf-8"), "secret")
        self.assertFalse(link.is_symlink())
        self.assertEqual(s.load_script("leak"), "result = 1\n")

    def test_script_dir_refuses_symlinked_final_directory(self) -> None:
        if not hasattr(os, "symlink"):
            self.skipTest("symlink not available on this platform")
        from pathlib import Path
        ...
        real = Path(self.tmp_home) / "real-scripts"
        real.mkdir()
        script_parent = Path(self.tmp_home) / ".config" / "axross"
        script_parent.mkdir(parents=True)
        os.symlink(real, script_parent / "scripts")

        with self.assertRaises(OSError):
            s.script_dir()

    def test_script_source_size_cap(self) -> None:
        from core import scripting as s

        too_large = "x" * (s.MAX_SCRIPT_SOURCE_BYTES + 1)
        with self.assertRaises(ValueError):
            s.save_script("big", too_large)

    def test_load_script_refuses_manually_planted_oversize_source(self) -> None:
        from pathlib import Path

        from core import scripting as s

        script_dir = Path(s.script_dir())
        (script_dir / "big.py").write_text("x" * 8, encoding="utf-8")
        with mock.patch.object(s, "MAX_SCRIPT_SOURCE_BYTES", 4):
            with self.assertRaises(OSError):
                s.load_script("big")

    def test_script_name_type_and_length_refused(self) -> None:
        from core import scripting as s

        with self.assertRaises(ValueError):
            s.save_script(None, "x = 1")  # type: ignore[arg-type]
        with self.assertRaises(ValueError):
            s.save_script("x" * (s.MAX_SCRIPT_NAME_CHARS + 1), "x = 1")

    def test_read_helpers_are_capped_by_default_but_allow_explicit_uncapped(self) -> None:
        from pathlib import Path

        from core import scripting as s

        with tempfile.TemporaryDirectory() as tmp:
            path = str(Path(tmp) / "payload.txt")
            Path(path).write_bytes(b"abcdef")
            backend = s.localfs()
            self.assertEqual(s.read_bytes(backend, path, max_bytes=6), b"abcdef")
            self.assertEqual(
                s.read_text(backend, path, max_bytes=6),
                "abcdef",
            )
            with self.assertRaises(OSError):
                s.read_bytes(backend, path, max_bytes=5)
            with self.assertRaises(OSError):
                s.read_text(backend, path, max_bytes=5)
            self.assertEqual(
                s.read_bytes(backend, path, max_bytes=None),
                b"abcdef",
            )

    def test_read_helpers_reject_negative_cap(self) -> None:
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(ValueError):
                s.read_bytes(s.localfs(), path, max_bytes=-1)
...
class RshBackendTests(unittest.TestCase):
    ...
    def test_list_dir_parses_ls_output(self) -> None:
        ...
        self.assertTrue(any("~" in cmd for cmd in recorded))
...
class PjlSafetyProbeTests(unittest.TestCase):
    ...
    def test_safety_probe_accepts_well_formed_pjl_reply(self) -> None:
        from core.pjl_client import UEL, PjlSession
    ...
    def test_safety_probe_refuses_silent_server(self) -> None:
        from core.pjl_client import PjlNotSupported, PjlSession
...
class NntpLibTests(unittest.TestCase):
    ...
    def test_rejects_oversized_single_line(self) -> None:
        from core import nntp_lib as N

        host, port, stop = self._fake_server(
            {
                "__greeting__": b"200 " + (b"x" * 128) + b"\r\n",
            }
        )
        try:
            with mock.patch.object(N, "MAX_LINE_BYTES", 32):
                with self.assertRaises(N.NntpError) as ctx:
                    N.NntpClient(host=host, port=port, use_tls=False)
            self.assertIn("line exceeds", str(ctx.exception))
        finally:
            stop()
...
class SqliteFsBackendTests(unittest.TestCase):
    ...
    def test_write_root_and_orphan_paths_are_refused(self) -> None:
        b, p = self._new_backend()
        try:
            with self.assertRaises(OSError):
                b.open_write("/")
            with self.assertRaises(OSError):
                b.open_write("/missing/file.txt")

            b.mkdir("/dir")
            with b.open_write("/dir/file.txt") as fh:
                fh.write(b"x")
            with self.assertRaises(OSError):
                b.rename("/dir/file.txt", "/missing/file.txt")
            with self.assertRaises(OSError):
                b.copy("/dir/file.txt", "/")
            self.assertEqual(
                b.open_read("/dir/file.txt").read(),
            ...
            self.assertEqual([i.name for i in b.list_dir("/")], ["dir"])
        finally:
            b.close()
            try:
                os.unlink(p)
...
class GopherBackendTests(unittest.TestCase):
    ...
    def test_parse_menu_handles_info_dir_file_and_terminator(self) -> None:
        from core.gopher_client import _entry_to_filename, _parse_menu
    ...
    def test_send_selector_rejects_crlf_smuggling(self) -> None:
        ...
        from core.gopher_client import GopherProtocolError, _send_selector
...
class ScpTransportPropertyTests(unittest.TestCase):
    """REGRESSION: ``ui.main_window`` checks
    ``hasattr(session, 'transport') and session.transport.is_active()``
    when wiring an SCP session into the terminal dock so the user
    can open a remote shell that multiplexes onto the same paramiko
    Transport. Without an exposed ``transport`` property on
    SCPSession, the gate fell through to a local shell."""

    @staticmethod
    def _make_scp_session(client=None):
        from core.scp_client import SCPSession

        sess = SCPSession.__new__(SCPSession)
        sess._client = client
        return sess

    def test_transport_property_is_present(self) -> None:
        sess = self._make_scp_session()
        self.assertTrue(
            hasattr(sess, "transport"),
            "SCPSession must expose .transport so the terminal-dock "
            "wire-up in main_window picks SCP sessions up",
        )

    def test_transport_returns_none_when_not_connected(self) -> None:
        sess = self._make_scp_session(client=None)
        self.assertIsNone(sess.transport)

    def test_transport_proxies_to_client_get_transport(self) -> None:
        ...
        sess = self._make_scp_session(client=fake_client)
        self.assertIs(sess.transport, fake_transport)
        fake_client.get_transport.assert_called_once()

    def test_ui_gate_passes_for_active_scp_session(self) -> None:
        # Replicate the exact gate the MainWindow uses to wire an
        # SSH/SCP session to the terminal dock.
        fake_transport = mock.MagicMock()
        fake_transport.is_active.return_value = True
        fake_client = mock.MagicMock()
        fake_client.get_transport.return_value = fake_transport
        sess = self._make_scp_session(client=fake_client)
        gate = hasattr(sess, "transport") and sess.transport and sess.transport.is_active()
        self.assertTrue(gate)

    def test_ui_gate_fails_when_transport_is_dead(self) -> None:
        fake_transport = mock.MagicMock()
...
class GDriveAppendTruncateGuardTests(unittest.TestCase):
    """REGRESSION: ``GDriveSession.open_write(append=True)`` must not
    silently truncate the existing file when the seed-with-existing-
    bytes step fails. Previously a transient read failure was
    swallowed by ``except OSError: pass`` — the writer then wrote
    only the appended tail, and the Drive update replaced the
    existing file with the new (smaller) content. Now: we re-raise
    after logging."""

    def test_append_raises_on_existing_read_failure(self) -> None:
        from core.gdrive_client import GDriveSession

        sess = GDriveSession.__new__(GDriveSession)
        sess._service = mock.MagicMock()

        # Stub everything _around_ the append-seed try-block.
        sess.normalize = lambda p: p
        sess.parent = lambda p: "/"
        sess._resolve_path = mock.MagicMock(
            side_effect=lambda p: "parent-id" if p == "/" else "existing-id"
        )

        # Stub _SpooledWriter so we don't actually exercise the Drive
        # API. The test is purely about error propagation.
        with mock.patch("core.gdrive_client._SpooledWriter") as fake_writer_cls:
            fake_writer_cls.return_value = mock.MagicMock()

            # open_read raises — this used to be silently swallowed.
            sess.open_read = mock.MagicMock(
                side_effect=OSError("permission denied"),
            )

            with self.assertLogs("core.gdrive_client", level="WARNING") as logs:
                with self.assertRaises(OSError):
                    sess.open_write("/foo.txt", append=True)

        joined = " ".join(logs.output).lower()
        self.assertIn("refusing to overwrite", joined)
        # The writer must NOT have been written to (existing bytes
        # never made it in, so the writer is in a half-seeded state
        # — raising prevents the half-seeded writer from reaching
        # the Drive API).
        self.assertEqual(fake_writer_cls.return_value.write.call_count, 0)


class ReplWidgetQtTests(unittest.TestCase):
    """End-to-end Qt-aware tests for ReplWidget. Uses the module-
    level QApplication singleton (test_hardening_regressions sets one
    up; we reuse it via QApplication.instance()).

    Covers behaviours the no-Qt tests below can't exercise:
    - keyPressEvent flow (Tab → completion, Up/Down → history,
      Enter → submit, Ctrl-D → re-prompt)
    - Multi-statement session source tracking
    ...
    def _make_widget(self):
        # Avoid touching the user's real history file: pass a
        # tmp-dir history path via monkey-patching HISTORY_PATH.
        import tempfile
        from pathlib import Path

        from ui import repl_widget

        tmp = tempfile.mkdtemp(prefix="axross-repl-test-")
        repl_widget.HISTORY_PATH = Path(tmp) / "repl_history"
        return repl_widget.ReplWidget()

    def test_session_source_is_empty_at_startup(self) -> None:
        w = self._make_widget()
        self.assertEqual(w._session_source, [])

    def test_submit_python_line_populates_session_source(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        # Type "1 + 1" then Enter via the synthetic key sequence.
        QTest.keyClicks(w, "1 + 1")
        QTest.keyClick(w, Qt.Key.Key_Return)
        self.assertIn("1 + 1", w._session_source)

    def test_submit_slash_command_excluded_from_session_source(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        QTest.keyClicks(w, ".help")
        QTest.keyClick(w, Qt.Key.Key_Return)
        # Help dispatched, no Python recorded for save.
        self.assertNotIn(".help", w._session_source)

    def test_ctrl_d_on_empty_prompt_re_prompts(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        # Buffer is empty, current input is empty.
        text_before = w.toPlainText()
        QTest.keyClick(w, Qt.Key.Key_D, Qt.KeyboardModifier.ControlModifier)
        text_after = w.toPlainText()
        # Must show the Ctrl-D message AND a fresh prompt — and a
        # new ">>> " must be visible at the end so the user isn't
        # left without an input line.
        self.assertTrue(text_before.endswith(">>> "))
        self.assertIn("Ctrl-D", text_after)
        self.assertTrue(text_after.endswith(">>> "), f"expected fresh prompt, got: {text_after!r}")

    def test_history_up_arrow_recalls_last_submission(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        QTest.keyClicks(w, "x = 42")
        QTest.keyClick(w, Qt.Key.Key_Return)
        # Now arrow-up should restore the line.
        QTest.keyClick(w, Qt.Key.Key_Up)
        self.assertEqual(w._current_input(), "x = 42")

    def test_tab_completion_axross_dot_offers_completions(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        QTest.keyClicks(w, "axross.localf")
        QTest.keyClick(w, Qt.Key.Key_Tab)
        # Single match → completed in place.
        self.assertEqual(w._current_input(), "axross.localfs")

    def test_tab_with_no_prefix_inserts_indent(self) -> None:
        from PyQt6.QtCore import Qt
        from PyQt6.QtTest import QTest

        w = self._make_widget()
        QTest.keyClick(w, Qt.Key.Key_Tab)
        # Pressed on empty prompt → 4-space indent
        self.assertTrue(
            w._current_input().endswith("    "),
            f"expected indent, got: {w._current_input()!r}",
        )


class ReplSaveSessionScopeTests(unittest.TestCase):
    """REGRESSION: ``.save`` in the embedded REPL used to dump
    everything in ``self._history`` — which is loaded from disk at
    startup and includes up to 1000 cumulative lines from prior
    sessions. The user expected just the current session's source.
    """

    def _make_widget_without_qt(self):
        # We can't instantiate ReplWidget without QApplication, so
        # we exercise the slash dispatcher logic by constructing a
        # bare object with the relevant attributes + bound method.
        from ui.repl_widget import ReplWidget

        w = ReplWidget.__new__(ReplWidget)
        w._history = []
        w._session_source = []
        w._buffer = []
        w._appended: list[str] = []
        w._append = w._appended.append  # capture .append() output
        return w

    def test_save_writes_only_session_source(self) -> None:
        w = self._make_widget_without_qt()
        # Simulate stale on-disk history loaded at startup.
        w._history = ["import os", "old_var = 1", "old_var + 2"]
        # And the THIS-session source the user actually wants saved.
        w._session_source = ["x = 42", "print(x)"]

        from ui.repl_widget import ReplWidget

        with mock.patch("core.scripting.save_script") as save:
            save.return_value = "/tmp/myscript.py"
            handled = ReplWidget._maybe_dispatch_slash(w, ".save myscript")

        self.assertTrue(handled)
        save.assert_called_once()
        saved_name, saved_src = save.call_args.args
        self.assertEqual(saved_name, "myscript")
        # Saved content must be ONLY the session source — not the
        # cross-session history.
        self.assertEqual(saved_src, "x = 42\nprint(x)\n")
        for stale in ("import os", "old_var"):
            self.assertNotIn(stale, saved_src, f"stale history line {stale!r} leaked into save")

    def test_save_with_empty_session_refuses(self) -> None:
        w = self._make_widget_without_qt()
        w._history = ["foo", "bar"]  # populated from disk
        w._session_source = []  # but THIS session is fresh

        from ui.repl_widget import ReplWidget

        with mock.patch("core.scripting.save_script") as save:
            handled = ReplWidget._maybe_dispatch_slash(w, ".save x")

        self.assertTrue(handled)
        save.assert_not_called()
        self.assertTrue(any("nothing to save" in s for s in w._appended))

    def test_save_filters_slash_commands_at_submit_time(self) -> None:
        # The submit path should NOT add slash-command lines to
        # _session_source. Verify by directly calling _submit_current's
        # gating logic.
        w = self._make_widget_without_qt()
        # Hand-rolled simulation of the gating in _submit_current.
        for line in (".help", "x = 1", ".save foo", "y = 2"):
            if line.strip() and not line.lstrip().startswith("."):
                w._session_source.append(line)
        self.assertEqual(w._session_source, ["x = 1", "y = 2"])


class ScriptingUiHelperTests(unittest.TestCase):
    """``axross.message`` / ``confirm`` / ``toast`` must work both
    with and without a running Qt event loop. Headless tests cover
    the stdout / stdin fallback path."""

    def test_message_headless_writes_to_stdout(self) -> None:
        from core.ui_helpers import message as ui_message

        with (
            mock.patch("core.ui_helpers._qt_is_running", return_value=False),
            mock.patch("builtins.print") as fake_print,
        ):
            ui_message("payload", title="Backup", level="success")
    ...
    def test_message_normalises_unknown_level(self) -> None:
        from core.ui_helpers import message as ui_message
        ...
        with (
            ...
            ui_message("p", level="banana")
        rendered = fake_print.call_args.args[0]
        self.assertIn("INFO", rendered)

    def test_confirm_headless_reads_from_stdin(self) -> None:
        from core.ui_helpers import confirm as ui_confirm

        with mock.patch("core.ui_helpers._qt_is_running", return_value=False):
            with mock.patch("builtins.input", return_value="y"):
                self.assertTrue(ui_confirm("?"))
            with mock.patch("builtins.input", return_value="ja"):
                self.assertTrue(ui_confirm("?"))
            with mock.patch("builtins.input", return_value="n"):
                self.assertFalse(ui_confirm("?"))
            # Empty answer → default
            with mock.patch("builtins.input", return_value=""):
                self.assertTrue(ui_confirm("?", default_yes=True))
                self.assertFalse(ui_confirm("?", default_yes=False))

    def test_confirm_eof_returns_default(self) -> None:
        # Headless fallback: stdin closed (e.g. cron / piped input
        # exhausted). Must not raise — returns the default answer.
        from core.ui_helpers import confirm as ui_confirm
    ...
    def test_toast_headless_writes_to_stdout(self) -> None:
        from core.ui_helpers import toast as ui_toast

        with (
            mock.patch("core.ui_helpers._qt_is_running", return_value=False),
            mock.patch("builtins.print") as fake_print,
        ):
            ui_toast("done", level="error")
        fake_print.assert_called_once()
        self.assertIn("ERROR", fake_print.call_args.args[0])

    def test_all_levels_have_svg_payload(self) -> None:
        from core.ui_helpers import _LEVEL_SVG, LEVELS

        for level in LEVELS:
            self.assertIn(level, _LEVEL_SVG)
            payload = _LEVEL_SVG[level]
            self.assertTrue(payload.startswith("<svg"))
            self.assertTrue(payload.endswith("</svg>"))

    def test_scripting_module_exposes_helpers(self) -> None:
        # Wire-up regression: the helpers must be reachable as
        # ``axross.message`` / ``axross.confirm`` / ``axross.toast``
        # AND listed in __all__ so the help-coverage gate passes.
        from core import scripting as ax

        self.assertTrue(hasattr(ax, "message"))
        self.assertTrue(hasattr(ax, "confirm"))
        self.assertTrue(hasattr(ax, "toast"))
        for name in ("message", "confirm", "toast"):
            self.assertIn(name, ax.__all__)


class ScriptingProxySupportTests(unittest.TestCase):
    """REGRESSION: ``axross.open_url(...)`` must accept proxy_type/
    proxy_host/proxy_port/proxy_username/proxy_password kwargs and
    route the connection through ConnectionManager so the proxy
    plumbing (SOCKS / HTTP CONNECT) actually applies. Without this
    the user couldn't tunnel one-shot script connections through a
    proxy without first creating a saved profile."""

    def test_proxy_kwargs_route_through_connection_manager(self) -> None:
        from core import scripting as ax

        # Capture the Profile that ConnectionManager.connect would
        # see — that's where the proxy fields live.
        captured = {}

        class _StubCM:
            def set_profile_resolver(self, _r):
                pass

            def connect(self, profile, password="", key_passphrase=""):
                captured["profile"] = profile
                captured["password"] = password
                captured["proxy_password"] = profile.get_proxy_password()
                return mock.MagicMock()

        with mock.patch("core.connection_manager.ConnectionManager", return_value=_StubCM()):
            ax.open_url(
        ...
        self.assertEqual(prof.protocol, "sftp")
        self.assertEqual(prof.host, "host.example.com")
        self.assertEqual(prof.username, "alice")
        self.assertEqual(prof.proxy_type, "socks5")
        self.assertEqual(prof.proxy_host, "proxy.example.com")
        self.assertEqual(prof.proxy_port, 1080)
        self.assertEqual(prof.proxy_username, "puser")
        self.assertEqual(captured["proxy_password"], "ppass")
        self.assertEqual(captured["password"], "hunter2")

    def test_open_url_refuses_proxy_type_without_host(self) -> None:
        from core import scripting as ax
    ...
    def test_open_url_refuses_proxy_type_without_port(self) -> None:
        from core import scripting as ax
        ...
        self.assertIn("proxy_port", str(ctx.exception))
        self.assertIn("refusing direct traffic", str(ctx.exception))

    def test_cm_protocol_without_proxy_still_routes_through_connection_manager(self) -> None:
        from core import scripting as ax
        ...
        with mock.patch("core.connection_manager.ConnectionManager", return_value=_StubCM()):
            ax.open_url("sftp://alice:pw@example.com/")

        prof = captured["profile"]
        self.assertEqual(prof.protocol, "sftp")
        self.assertEqual(prof.host, "example.com")
        self.assertEqual(prof.port, 22)
        self.assertEqual(prof.username, "alice")
        self.assertEqual(captured["password"], "pw")

    def test_webdav_http_url_maps_to_profile_url_without_userinfo(self) -> None:
        from core import scripting as ax
        ...
        with mock.patch("core.connection_manager.ConnectionManager", return_value=_StubCM()):
            ax.open_url("https://alice:pw@dav.example/remote.php/dav/")

        prof = captured["profile"]
        self.assertEqual(prof.protocol, "webdav")
        self.assertEqual(prof.webdav_url, "https://dav.example/remote.php/dav/")
        self.assertEqual(prof.username, "alice")
        self.assertEqual(captured["password"], "pw")

    def test_profile_url_components_seed_protocol_specific_fields(self) -> None:
        from core import scripting as ax
        ...
        with mock.patch("core.connection_manager.ConnectionManager", return_value=_StubCM()):
            ax.open_url(
                "s3://bucket-name", proxy_type="http", proxy_host="proxy.example", proxy_port=8080
            )
            ax.open_url("smb://fileserver/teamshare")
            ax.open_url("rsync://rsync.example/module")
            ax.open_url("nfs://nas.example/export/projects")

        self.assertEqual(captured[0].s3_bucket, "bucket-name")
        self.assertEqual(captured[1].smb_share, "teamshare")
        self.assertEqual(captured[2].rsync_module, "module")
        self.assertEqual(captured[3].nfs_export, "/export/projects")

    def test_git_local_url_maps_to_path_not_remote_url(self) -> None:
        from core import scripting as ax
        ...
        with mock.patch("core.backend_registry.load_backend_class", return_value=_FakeGit):
            ax.open_url("git+local:///srv/repos/foo.git")

        self.assertEqual(calls["path"], "/srv/repos/foo.git")
        self.assertNotIn("url", calls)

    def test_no_proxy_kwargs_uses_direct_construction(self) -> None:
        # When the user does NOT pass proxy kwargs, open_url should
        # still use the original direct-construct path so backends
        # ConnectionManager doesn't handle (gopher, sqlite, etc.)
        # keep working.
        from core import scripting as ax
        ...
        # Use sqlite (handled by direct construction, not CM) without
        # proxy → must work as before.
        with mock.patch("core.connection_manager.ConnectionManager") as cm:
            sess = ax.open_url("sqlite:///:memory:")
            cm.assert_not_called()
            self.assertIsNotNone(sess)

    def test_url_components_seed_profile_when_missing_kwargs(self) -> None:
        # URL: ftp://user:pass@host:2121/  with explicit proxy
        from core import scripting as ax
        ...
        with mock.patch("core.connection_manager.ConnectionManager", return_value=_StubCM()):
            ax.open_url(
                "ftp://bob:s3cret@example.com:2121/",
                proxy_type="http",
                proxy_host="proxy.example.com",
                proxy_port=8080,
            )
        prof = captured["profile"]
        self.assertEqual(prof.protocol, "ftp")
        self.assertEqual(prof.host, "example.com")
        self.assertEqual(prof.port, 2121)
        self.assertEqual(prof.username, "bob")
        # The password from the URL was unquoted and forwarded to
        # ConnectionManager.connect — NOT stored in the Profile (we
        # never persist URL-borne creds).
        self.assertEqual(captured["password"], "s3cret")
        self.assertEqual(prof.proxy_type, "http")


class SvnFsSessionTests(unittest.TestCase):
    """Coverage for the new read-only SVN FileBackend (subprocess
    wrapper around the ``svn`` CLI). Subprocess is fully mocked so
    these tests don't need svn installed."""

    @staticmethod
    def _ls_xml(entries: list[tuple[str, str, int]]) -> bytes:
        """Build a minimal ``svn ls --xml`` payload. Each entry is
        ``(name, kind, size)`` where kind is 'file' or 'dir'."""
        body = "<?xml version='1.0'?><lists><list path='url'>"
        for name, kind, size in entries:
            body += (
                f"<entry kind='{kind}'>"
                f"<name>{name}</name>"
                f"<size>{size}</size>"
                f"<commit revision='42'>"
                f"<author>alice</author>"
                f"<date>2026-04-26T10:00:00.123456Z</date>"
                f"</commit>"
                f"</entry>"
            )
        body += "</list></lists>"
    ...
    def _info_xml() -> bytes:
        return (
            ...
            b"<entry kind='dir' revision='42'>"
            b"<repository><root>https://svn.example.com/repo</root></repository>"
            b"</entry></info>"
        )

    def _make_session(self, ls_xml: bytes | None = None):
        from core import svn_fs_client as svn_mod

        with (
            mock.patch("core.svn_fs_client._svn_available", return_value=True),
            mock.patch("subprocess.run") as run,
        ):
            run.return_value = mock.MagicMock(
                returncode=0,
                stdout=self._info_xml(),
                stderr=b"",
            )
            sess = svn_mod.SvnFsSession(
    ...
    def test_init_requires_svn_binary(self) -> None:
        from core import svn_fs_client as svn_mod

        with mock.patch("core.svn_fs_client._svn_available", return_value=False):
            with self.assertRaises(ImportError):
                svn_mod.SvnFsSession(url="https://x/y")
    ...
    def test_init_requires_url(self) -> None:
        ...
        with (
            ...
            with self.assertRaises(OSError):
                svn_mod.SvnFsSession(url="")

    def test_password_auth_uses_noninteractive_no_auth_cache(self) -> None:
        # The portable svn CLI only exposes password auth as
        # ``--password``. Keep it non-interactive and out of svn's
        # on-disk auth cache, but be honest that argv contains it.
        sess, svn_mod = self._make_session()
        argv = sess._build_argv("ls", "https://svn.example.com/repo")
        self.assertIn("--password", argv)
        self.assertIn("s3cret", argv)
        self.assertIn("--non-interactive", argv)
        self.assertIn("--no-auth-cache", argv)

    def test_url_embedded_password_is_stripped_from_stored_url(self) -> None:
        from core import svn_fs_client as svn_mod
        ...
        with (
            ...
            with self.assertLogs("core.svn_fs_client", level="WARNING") as logs:
                sess = svn_mod.SvnFsSession(
                    url=("https://al%69ce:p%40ss%20word@svn.example.com:8443/repo"),
                )
        self.assertEqual(sess._url, "https://svn.example.com:8443/repo")
        self.assertEqual(sess._username, "alice")
        self.assertEqual(sess._password, "p@ss word")
        self.assertNotIn("p%40ss", sess.name)
        self.assertNotIn("p@ss", sess.name)
        first_argv = run.call_args_list[0].args[0]
        joined = " ".join(first_argv)
        self.assertIn("https://svn.example.com:8443/repo", first_argv)
        self.assertNotIn("al%69ce:p%40ss%20word@", joined)
        self.assertTrue(any("process inspectors" in m for m in logs.output))

    def test_svn_info_rejects_doctype_xml_when_defusedxml_available(self) -> None:
        from core import svn_fs_client as svn_mod

        if "defusedxml" not in getattr(
            svn_mod._xml_fromstring,
            "__module__",
            "",
        ):
            self.skipTest("defusedxml not installed")
        hostile = (
            b"<!DOCTYPE info [<!ENTITY xxe SYSTEM 'file:///etc/passwd'>]>"
            b"<info><entry kind='dir' revision='1'>&xxe;</entry></info>"
        )
        with (
            mock.patch("core.svn_fs_client._svn_available", return_value=True),
            mock.patch("subprocess.run") as run,
            ...
            with self.assertRaises(OSError) as ctx:
                svn_mod.SvnFsSession(url="https://svn.example.com/repo")
        self.assertIn("malformed XML", str(ctx.exception))

    def test_list_dir_parses_xml_entries(self) -> None:
        sess, svn_mod = self._make_session()
        ls_xml = self._ls_xml(
            [
                ("README.md", "file", 1234),
        ...
        # File sizes preserved; commit author exposed as owner.
        for it in items:
            if it.name == "README.md":
                self.assertEqual(it.size, 1234)
                self.assertEqual(it.owner, "alice")

    def test_open_read_returns_blob(self) -> None:
        sess, svn_mod = self._make_session()
        with mock.patch("subprocess.run") as run:
            run.return_value = mock.MagicMock(
                returncode=0,
            ...
            handle = sess.open_read("/README.md")
        self.assertEqual(handle.read(), b"hello world\n")

    def test_open_read_rejects_oversized_blob(self) -> None:
        sess, svn_mod = self._make_session()
        # Patch _MAX_CAT_BYTES down to something testable.
        with mock.patch.object(svn_mod, "_MAX_CAT_BYTES", 16), mock.patch("subprocess.run") as run:
            run.return_value = mock.MagicMock(
                returncode=0,
                stdout=b"x" * 32,
                stderr=b"",
            ...
            with self.assertRaises(OSError) as cm:
                sess.open_read("/big.bin")
        self.assertIn("read cap", str(cm.exception))
    ...
    def test_writes_are_refused(self) -> None:
        ...
        for op in (
            ...
            lambda: sess.mkdir("/x"),
            lambda: sess.rmdir("/x"),
            lambda: sess.rename("/x", "/y"),
            lambda: sess.chmod("/x", 0o644),
        ):
            with self.assertRaises(OSError):
                op()

    def test_subprocess_failure_surfaces_as_oserror(self) -> None:
        sess, svn_mod = self._make_session()
        with mock.patch("subprocess.run") as run:
            run.return_value = mock.MagicMock(
                returncode=1,
                stdout=b"",
                stderr=b"svn: E170000: URL not found",
            )
            with self.assertRaises(OSError) as cm:
                sess.list_dir("/missing")
        self.assertIn("E170000", str(cm.exception))

    def test_subprocess_timeout_surfaces_as_oserror(self) -> None:
        sess, svn_mod = self._make_session()
        with mock.patch(
            "subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="svn", timeout=60.0)
        ):
            with self.assertRaises(OSError) as cm:
                sess.list_dir("/anything")
        self.assertIn("timed out", str(cm.exception).lower())

    def test_revision_pin_propagates_to_argv(self) -> None:
        from core import svn_fs_client as svn_mod
        ...
        with (
            ...
            run.return_value = mock.MagicMock(
                returncode=0,
                stdout=self._ls_xml([("foo", "file", 1)]),
                stderr=b"",
            )
            sess.list_dir("/")
        # Last call to subprocess.run should include --revision 42.
        last_argv = run.call_args.args[0]
        self.assertIn("--revision", last_argv)
        idx = last_argv.index("--revision")
        self.assertEqual(last_argv[idx + 1], "42")


# Need subprocess at import time for the timeout test above.
