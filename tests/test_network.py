...
Network layout:
  ...
  socks-proxy 10.99.0.20:1080 (SOCKS4/SOCKS5, no auth)
...
SOCKS_PORT = 1080
SOCKS4_HOST = SOCKS_HOST
SOCKS4_PORT = SOCKS_PORT
SOCKS5_HOST = SOCKS_HOST
...
def _ssh_lab_available() -> bool:
    """Cheap TCP probe to the SSH-alpha endpoint.

    Mirrors `test_protocols._protocol_lab_available()`. Used to
    short-circuit the whole module when the Docker SSH lab is not
    started — otherwise each `connect()` falls into a multi-second
    paramiko handshake timeout, multiplied by ~38 testcases, which
    blows past the audit-gate per-suite timeout. Mocked SSH coverage
    in `tests/test_hardening_regressions.py` and
    `tests/test_backend_regressions.py` protects production logic
    when the live lab is offline.
    """
    alpha = SERVERS["alpha"]
    try:
        with socket.create_connection((alpha["host"], alpha["port"]), timeout=0.25):
            return True
    except OSError:
        return False


if not _ssh_lab_available():
    pytest.skip(
        "Docker SSH lab endpoints (10.99.0.10-12) unreachable; mocked "
        "SSH coverage in test_hardening_regressions.py and "
        "test_backend_regressions.py protects production logic when "
        "the live lab is offline. Start the lab with "
        "`cd tests/docker && docker compose up -d` to run these.",
...
def test_ssh_1gb_canary():
    ...
    if os.environ.get("AXROSS_RUN_1GB_CANARY") != "1":
        import pytest

        pytest.skip("set AXROSS_RUN_1GB_CANARY=1 to run the 1 GB SFTP canary")
    import hashlib
    import struct
...
# ════════════════════════════════════════════════════════════
#  SECTION 3: SOCKS proxy
# ════════════════════════════════════════════════════════════


@_label("3.0a SOCKS4 proxy to ssh-alpha")
def test_socks4_alpha():
    profile = make_profile(
        "alpha",
        proxy_type="socks4",
        proxy_host=SOCKS4_HOST,
...
@_label("3.0b SOCKS4 proxy — raw socket creation")
def test_socks4_raw_socket():
    proxy = ProxyConfig(
        proxy_type="socks4",
        host=SOCKS4_HOST,
        port=SOCKS4_PORT,
...
def test_cm_multi_server():
    ...
    try:
        for name, p in profiles.items():
            sessions[name] = cm.connect(
                p, password=SERVERS[name]["pass"], on_unknown_host=auto_trust
            )
...
def test_unknown_host_callback():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        # Use a temp known_hosts path so the key is definitely unknown.
        session._known_hosts_path = Path(tmp) / "known_hosts"
        try:
...
def test_host_key_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_known = Path(tmp) / "known_hosts"
        ...
        try:
            session1.connect(
                password=SERVERS["beta"]["pass"],
                on_unknown_host=lambda e: True,
            )
        finally:
            session1.disconnect()
        ...
        try:
            session2.connect(
                password=SERVERS["beta"]["pass"],
                on_unknown_host=should_not_be_called,
            )
