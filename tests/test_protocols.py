...
SOCKS4_PROXY_HOST = SOCKS_PROXY_HOST
SOCKS4_PROXY_PORT = SOCKS_PROXY_PORT
SOCKS5_PROXY_HOST = SOCKS_PROXY_HOST
SOCKS5_PROXY_PORT = SOCKS_PROXY_PORT
HTTP_PROXY_HOST = "10.99.0.21"
HTTP_PROXY_PORT = 8888


def _protocol_lab_available() -> bool:
    try:
        with socket.create_connection((FTP_HOST, FTP_PORT), timeout=0.25):
            return True
    except OSError:
        return False
...
def test_ftp_connect():
    ...
    assert len(names) > 0, "Expected files, got empty listing"
    s.disconnect()
...
def test_ftp_list_data():
    ...
    s = FtpSession(host=FTP_HOST, port=FTP_PORT, username=FTP_USER, password=FTP_PASS)
    # Find the data dir — may be at ~/data or relative
    home = s.home()
...
def test_ftp_paths():
    ...
    s = FtpSession(host=FTP_HOST, port=FTP_PORT, username=FTP_USER, password=FTP_PASS)
    assert s.separator() == "/"
    assert s.join("/home", "user") == "~"
    assert s.parent("~/data") == "~"
    s.disconnect()
...
def test_ftps_connect():
    ...
    try:
        assert s.connected
        items = s.list_dir(s.home())
        assert any(i.name == "data" for i in items), f"'data' not in {[i.name for i in items]}"
    finally:
        s.disconnect()
...
def test_smb_connect():
    ...
    s = SmbSession(
        host=SMB_HOST, share=SMB_SHARE, username=SMB_USER, password=SMB_PASS, port=SMB_PORT
    )
    assert s.connected
    items = s.list_dir("/")
    names = {i.name for i in items}
...
def _can_mount_nfs() -> bool:
    ...
    try:
        r = subprocess.run(
            ["mount", "-t", "nfs", "-o", f"vers=4,port={NFS_PORT},nolock,tcp", f"{NFS_HOST}:/", mp],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
class TestIscsi:
    ...
    def setup_class(cls):
            ...
            with open("/proc/mounts") as fh:
                for line in fh:
                    ...
                    if len(parts) >= 2 and parts[0].startswith("/dev/sd"):
                        subprocess.run(["umount", "-f", parts[1]], capture_output=True, timeout=5)
        except OSError:
            pass
        ...
        for _ in range(3):
            r = subprocess.run(
                ["iscsiadm", "-m", "session"], capture_output=True, text=True, timeout=5
            )
            sids = []
        ...
        subprocess.run(
            ["iscsiadm", "-m", "node", "-o", "delete", "-T", ISCSI_IQN],
            capture_output=True,
            timeout=5,
        )
        ...
        for _ in range(10):
            r = subprocess.run(
                ["iscsiadm", "-m", "session"], capture_output=True, text=True, timeout=3
            )
    ...
    def test_iscsi_readlink_chain_loop_is_detected(self):
        ...
        try:
            ...
            try:
                ...
                raise AssertionError("open_read through a symlink loop should raise OSError")
        finally:
            for p in (a, b):
                if _os.path.lexists(p):
                    _os.remove(p)
...
def _imap_session():
    ...
    return ImapSession(
        host=IMAP_HOST, port=IMAP_PORT, username=IMAP_USER, password=IMAP_PASS, use_ssl=False
    )
...
def _telnet_session():
    ...
    return TelnetSession(
        host=TELNET_HOST, port=TELNET_PORT, username=TELNET_USER, password=TELNET_PASS
    )
    ...
    def test_telnet_binary_roundtrip(self):
        ...
        try:
            ...
            with s.open_read(test_path) as f:
                read_back = f.read()
            assert read_back == test_data, (
                f"Binary mismatch: wrote {len(test_data)} bytes, read {len(read_back)} bytes"
            )
            s.remove(test_path)
        finally:
            s.disconnect()
...
class TestAzureBlob:
    ...
    def setup_class(cls):
        ...
        container.upload_blob("data/bigfile.bin", os.urandom(64 * 1024), overwrite=True)
...
def test_ftp_wrong_password():
    ...
    _expect_auth_or_connection_error(
        lambda: FtpSession(
            host=FTP_HOST, port=FTP_PORT, username=FTP_USER, password="wro" + "ng-" + FTP_PASS
        ),
        "FTP wrong password",
    )
...
def test_smb_wrong_password():
    ...
    _expect_auth_or_connection_error(
        lambda: SmbSession(
            host=SMB_HOST, share=SMB_SHARE, username=SMB_USER, password="bogus", port=SMB_PORT
        ),
        "SMB wrong password",
    )
...
def test_imap_wrong_password():
    ...
    _expect_auth_or_connection_error(
        lambda: ImapSession(
            host=IMAP_HOST, port=IMAP_PORT, username=IMAP_USER, password="bogus", use_ssl=False
        ),
        "IMAP wrong password",
    )
...
def test_telnet_wrong_password():
    ...
    _expect_auth_or_connection_error(
        lambda: TelnetSession(
            host=TELNET_HOST, port=TELNET_PORT, username=TELNET_USER, password="bogus"
        ),
...
def test_azure_blob_wrong_credentials():
    ...
    _expect_auth_or_connection_error(
        lambda: AzureBlobSession(connection_string=bad_conn, container=AZURITE_CONTAINER),
...
def test_ftp_unreachable():
    ...
    _expect_auth_or_connection_error(
        lambda: FtpSession(
            host=_UNREACHABLE_HOST, port=_UNREACHABLE_PORT, username="u", password="p"
        ),
        "FTP unreachable",
    )
...
def test_smb_unreachable():
    ...
    _expect_auth_or_connection_error(
        lambda: SmbSession(
            host=_UNREACHABLE_HOST, port=_UNREACHABLE_PORT, share="x", username="u", password="p"
        ),
        "SMB unreachable",
    )
...
def _unicode_roundtrip(session, dir_path="/"):
    ...
    path = (
        session.join(dir_path, _UNICODE_FILENAME)
        if hasattr(session, "join")
        else f"{dir_path.rstrip('/')}/{_UNICODE_FILENAME}"
    )
    try:
...
def _empty_file_roundtrip(s, dir_path):
    path = (
        s.join(dir_path, "empty_rt.txt")
        if hasattr(s, "join")
        else f"{dir_path.rstrip('/')}/empty_rt.txt"
    )
    try:
        with s.open_write(path) as f:
            f.write(b"")
...
def _toxiproxy_create(name: str, upstream_host: str, upstream_port: int, listen_port: int) -> str:
    ...
    try:
        req = urllib.request.Request(f"{TOXIPROXY_ADMIN}/proxies/{name}", method="DELETE")
...
def _toxiproxy_add_toxic(
    proxy: str, toxic_name: str, toxic_type: str, attributes: dict, stream: str = "downstream"
) -> None:
...
def test_imap_mid_transfer_drop():
    ...
    try:
        ...
        s = ImapSession(
            host=TOXIPROXY_HOST, port=10143, username=IMAP_USER, password=IMAP_PASS, use_ssl=False
        )
        try:
...
def test_imap_high_latency():
    ...
    try:
        ...
        _toxiproxy_add_toxic(PROXY, "lag", "latency", {"latency": 200, "jitter": 50})
        s = ImapSession(
            host=TOXIPROXY_HOST, port=10144, username=IMAP_USER, password=IMAP_PASS, use_ssl=False
        )
        try:
            t0 = _t.monotonic()
            items = s.list_dir("/")
...
def test_webdav_mid_transfer_drop():
    ...
    try:
        ...
        s = WebDavSession(url=proxied_url, username=WEBDAV_USER, password=WEBDAV_PASS)
        try:
            s.list_dir("/")
...
def test_smb_mid_transfer_drop():
    ...
    try:
        ...
        s = SmbSession(
            host=TOXIPROXY_HOST, share=SMB_SHARE, username=SMB_USER, password=SMB_PASS, port=445
        )
        try:
...
def _isolate_known_hosts(session, label: str):
    """Keep toxiproxy SSH aliases out of the user's known_hosts file.

    The same local toxiproxy ports may be reused by different test runs
    while containers are recreated, so the host key pinning signal would
    otherwise become order-dependent.
    """
    safe = "".join(ch if ch.isalnum() else "_" for ch in label)
    path = Path(tempfile.gettempdir()) / (f"axross-test-known-hosts-{os.getpid()}-{safe}")
    try:
        path.unlink()
    except FileNotFoundError:
        pass
    session._known_hosts_path = path
    return session
...
def test_ssh_mid_transfer_drop():
    ...
    try:
        ...
        s = _isolate_known_hosts(SSHSession(profile), PROXY)
...
def test_telnet_mid_transfer_drop():
    ...
    try:
        ...
        s = TelnetSession(
            host=TOXIPROXY_HOST, port=10037, username=TELNET_USER, password=TELNET_PASS
        )
        try:
            s.list_dir(s.home())
            _toxiproxy_disable(PROXY)
...
def test_azure_blob_mid_transfer_drop():
    ...
    try:
        ...
        s = AzureBlobSession(connection_string=proxied_conn, container=AZURITE_CONTAINER)
        try:
            s.list_dir("/")
            _toxiproxy_disable(PROXY)
            try:
...
def test_ftp_mid_transfer_drop():
    ...
    try:
        ...
        s = FtpSession(host=TOXIPROXY_HOST, port=10022, username=FTP_USER, password=FTP_PASS)
        try:
...
def test_ssh_bandwidth_cap():
    ...
    try:
        ...
        s.connect(password=_SSH_PASS, on_unknown_host=_auto_trust)
        try:
            # ssh-alpha image ships data/bigfile.bin; read only one
            # 64 KB chunk so the 32 KB/s toxic keeps the test bounded.
...
def test_telnet_bandwidth_cap():
    ...
    try:
        ...
        _toxiproxy_add_toxic(PROXY, "slow", "bandwidth", {"rate": 32})
        s = TelnetSession(
            host=TOXIPROXY_HOST, port=11037, username=TELNET_USER, password=TELNET_PASS
        )
        try:
            remote = s.join(s.home(), "data", "bigfile.bin")
            t0 = _t.monotonic()
...
def test_ssh_high_latency():
    ...
    try:
        ...
        _toxiproxy_add_toxic(PROXY, "lag", "latency", {"latency": 150, "jitter": 50})
...
def test_webdav_high_latency():
    ...
    try:
        ...
        _toxiproxy_add_toxic(PROXY, "lag", "latency", {"latency": 150, "jitter": 50})
        s = WebDavSession(
            url=f"http://{TOXIPROXY_HOST}:12080", username=WEBDAV_USER, password=WEBDAV_PASS
        )
        try:
            t0 = _t.monotonic()
            items = s.list_dir("/")
...
def test_ftps_rejects_self_signed_by_default():
    ...
    try:
        FtpSession(host=FTPS_HOST, port=FTPS_PORT, username=FTPS_USER, password=FTPS_PASS, tls=True)
    except ssl.SSLError:
        return
        ...
        raise
    raise AssertionError(
        "FtpSession with verify_tls=True accepted a self-signed cert — MITM guard is broken"
    )
...
def test_ftps_rejects_hostname_mismatch():
    ...
    try:
        try:
            FtpSession(
                host=TOXIPROXY_HOST, port=10021, username=FTPS_USER, password=FTPS_PASS, tls=True
            )
...
def test_ftp_disk_full_raises():
    ...
    s = FtpSession(host=TINY_FTP_HOST, port=21, username=TINY_FTP_USER, password=TINY_FTP_PASS)
    try:
    ...
    try:
        for bad in ("value\x00with-nul", "value\r\nLOG-INJECT", "value\nNEWLINE"):
            try:
                ...
                raise AssertionError(f"env value {bad!r} should be refused")
    finally:
        s.disconnect()
...
def test_ssh_interactive_shell_multiplex_with_sftp():
    ...
    transport. Both must work concurrently — that's the whole point
    of the feature."""
    from core.profiles import ConnectionProfile
    from core.ssh_client import SSHSession
    ...
    s.connect(password=_SSH_PASS, on_unknown_host=_auto_trust)
    try:
        # SFTP works at session level.
        before = s.list_dir(s.home())
        assert len(before) > 0

        with s.interactive_shell(cols=120, rows=40) as sh:
            # Drain prompt
            sh.read_until(b"$ ", timeout=5.0)
            # Echo a unique marker through the shell
            marker = b"AXROSS-MUX-MARKER-7331"
            sh.write(b"echo " + marker + b"\n")
            out = sh.read_until(marker, timeout=5.0)
            assert marker in out, out[-200:]
            # CONCURRENT SFTP — same transport, different channel
            during = s.list_dir(s.home())
            assert [i.name for i in during] == [i.name for i in before]

        # Channel was closed; transport still alive
        assert s.connected
        after = s.list_dir(s.home())
        assert [i.name for i in after] == [i.name for i in before]
    finally:
        s.disconnect()
...
def test_ssh_interactive_shell_env_and_term_validation():
    ...
    s.connect(password=_SSH_PASS, on_unknown_host=_auto_trust)
    try:
        # Env-key allow-list (same gate as exec()).
        for bad_key in ("FOO BAR", "1ABC", "X-Y"):
            try:
                with s.interactive_shell(env={bad_key: "v"}):
                    pass
            except OSError as e:
                assert "[A-Za-z_][A-Za-z0-9_]*" in str(e)
            else:
                raise AssertionError(f"env key {bad_key!r} should be refused")
        # Env-value NUL/CR/LF (F39 symmetry).
        for bad_val in ("v\x00x", "v\r\nLOG", "v\nx"):
            try:
                with s.interactive_shell(env={"OK": bad_val}):
                    pass
            except OSError as e:
                assert "NUL/CR/LF" in str(e)
            else:
                raise AssertionError(f"env value {bad_val!r} should be refused")
        # term must not contain NUL/CR/LF.
        for bad_term in ("xterm\nfoo", "xterm\rbar", "xterm\x00"):
            try:
                with s.interactive_shell(term=bad_term):
                    pass
            except ValueError as e:
                assert "NUL/CR/LF" in str(e)
            else:
                raise AssertionError(f"term {bad_term!r} should be refused")
    finally:
        s.disconnect()
...
def test_scp_interactive_shell_multiplex():
    ...
    try:
        before = s.list_dir(s.home())
        with s.interactive_shell() as sh:
            sh.read_until(b"$ ", timeout=5.0)
            sh.write(b"whoami\n")
            out = sh.read_until(b"$ ", timeout=5.0)
            assert _SSH_USER.encode() in out, out[-200:]
            during = s.list_dir(s.home())
            assert [i.name for i in during] == [i.name for i in before]
    finally:
        s.disconnect()
...
def test_axross_interactive_shell_dispatch():
    ...
    s.connect(password=_SSH_PASS, on_unknown_host=_auto_trust)
    try:
        with axross.interactive_shell(s) as sh:
            sh.read_until(b"$ ", timeout=5.0)
            sh.write(b"echo dispatched\n")
            out = sh.read_until(b"dispatched", timeout=5.0)
            assert b"dispatched" in out
    finally:
        s.disconnect()
    # localfs must refuse loudly
    b = axross.localfs()
    try:
        axross.interactive_shell(b)
    except TypeError as e:
        assert "interactive_shell" in str(e)
    else:
        raise AssertionError("interactive_shell on localfs should TypeError")
...
def test_azure_blob_checksum_md5():
    ...
    s = AzureBlobSession(connection_string=AZURITE_CONNECTION_STRING, container=AZURITE_CONTAINER)
    try:
        cs = s.checksum(f"/{key}")
        _assert_checksum_shape(cs)
        assert cs.startswith("md5:"), cs
...
def _ops_matrix_session(protocol: str):
    if protocol == "sftp":
        from core.profiles import ConnectionProfile
        from core.ssh_client import SSHSession

        _require_lab_endpoint("SSH alpha", _SSH_HOST, _SSH_PORT)
        s = SSHSession(
            ConnectionProfile(
                name="ops-sftp",
                protocol="sftp",
    ...
    if protocol == "ftp":
        ...
        _require_lab_endpoint("FTP", FTP_HOST, FTP_PORT)
        return FtpSession(
            host=FTP_HOST,
            port=FTP_PORT,
            username=FTP_USER,
    ...
    if protocol == "ftps":
        ...
        _require_lab_endpoint("FTPS", FTPS_HOST, FTPS_PORT)
        return FtpSession(
            host=FTPS_HOST,
            port=FTPS_PORT,
            username=FTPS_USER,
    ...
    if protocol == "smb":
        ...
        _require_lab_endpoint("SMB", SMB_HOST, SMB_PORT)
        return SmbSession(
            host=SMB_HOST,
            share=SMB_SHARE,
            username=SMB_USER,
    ...
    if protocol == "webdav":
        ...
        _require_lab_endpoint("WebDAV", WEBDAV_HOST, WEBDAV_PORT)
    ...
    if protocol == "s3":
        ...
        _require_lab_endpoint("S3/MinIO", S3_HOST, S3_PORT)
        return S3Session(
            bucket=S3_BUCKET,
            endpoint=S3_ENDPOINT,
            access_key=S3_ACCESS_KEY,
    ...
    if protocol == "rsync":
        ...
        _require_lab_endpoint("Rsync", RSYNC_HOST, RSYNC_PORT)
        return RsyncSession(
            host=RSYNC_HOST,
            port=RSYNC_PORT,
            module=RSYNC_MODULE,
    ...
    if protocol == "telnet":
        _require_lab_endpoint("Telnet", TELNET_HOST, TELNET_PORT)
        return _proxied_telnet_session("none", "", 0)
    if protocol == "azure_blob":
        from core.azure_client import AzureBlobSession
        ...
        try:
            ...
            pytest.skip(f"azure-storage-blob not installed: {exc}")
        _require_lab_endpoint("Azurite", AZURITE_HOST, 10000)
        ...
        try:
            ...
            pass
        return AzureBlobSession(
            connection_string=AZURITE_CONNECTION_STRING,
            container=AZURITE_CONTAINER,
        )
    raise AssertionError(f"unknown ops matrix protocol: {protocol}")


def _disconnect_quietly(session) -> None:
...
@pytest.mark.parametrize("protocol", _OPS_MATRIX_PROTOCOLS)
def test_filebackend_open_copy_rename_mkdir_matrix(protocol):
    from core.server_ops import server_side_copy, server_side_move

    s = _ops_matrix_session(protocol)
    base = (
        s.join(s.home(), f"ops_matrix_{protocol}")
        if protocol in {"sftp", "ftp", "ftps", "telnet"}
        else f"/ops_matrix_{protocol}"
    )
    payload = f"open/copy/rename/mkdir contract: {protocol}\n".encode("ascii")
    src = s.join(base, "open-src.txt")
    copied = s.join(base, "copied.txt")
    renamed = s.join(base, "renamed.txt")
    try:
        try:
            if s.exists(base):
            ...
            pass
        s.mkdir(base)
        assert s.exists(base) and s.is_dir(base)

        with s.open_write(src) as fh:
            fh.write(payload)
        with s.open_read(src) as fh:
            assert fh.read() == payload

        server_side_copy(s, src, copied)
        with s.open_read(copied) as fh:
            assert fh.read() == payload
        assert s.exists(src) and s.exists(copied)

        server_side_move(s, copied, renamed)
        assert not s.exists(copied)
        assert s.exists(renamed)
        with s.open_read(renamed) as fh:
            assert fh.read() == payload
    finally:
        try:
            if s.exists(base):
            ...
            for path in (src, copied, renamed):
                try:
                    if s.exists(path):
                        s.remove(path)
                except Exception:
...
def test_s3_list_versions():
    ...
    try:
        ...
        for v in versions:
            try:
                s._s3.delete_object(Bucket=S3_BUCKET, Key=key[1:], VersionId=v.version_id)
            except Exception:
                pass
        s.disconnect()
...
def test_ftp_trash_roundtrip():
    ...
    s = FtpSession(host=FTP_HOST, port=FTP_PORT, username=FTP_USER, password=FTP_PASS)
    try:
        # ~ is writable; /data is root-owned. Write under home.
        home = s.home()
...
def test_smb_trash_directory():
    ...
    s = SmbSession(
        host=SMB_HOST, port=SMB_PORT, share=SMB_SHARE, username=SMB_USER, password=SMB_PASS
    )
    try:
        base = s.home()
...
def test_ftp_xlink_roundtrip():
    ...
    try:
        ...
        final = X.create_xlink(s, link_name, target, display_name="remote doc")
...
def _name_roundtrip(s, base, name: str) -> str:
    ...
    try:
        ...
        for n in listed:
            if n in candidates:
                return n
        raise AssertionError(f"name {name!r} not in listing (as NFC/NFD). Got: {listed}")
    finally:
        try:
            s.remove(path)
        except OSError:
            ...
            for alt in (_uni.normalize("NFC", name), _uni.normalize("NFD", name)):
                try:
...
def test_smb_unicode_case_insensitive():
    ...
    try:
        ...
        try:
            ...
            assert s.exists(lower_path), f"SMB case-insensitive exists failed for {lower_path}"
        finally:
            s.remove(path)
    finally:
        s.disconnect()
...
def test_sftp_partial_read():
    pytest.skip(
        "SFTP is exercised in test_network.py; no sftp container in this suite's lab harness"
    )
...
#  section first validates each proxy protocol directly (SOCKS4,
#  SOCKS5, HTTP CONNECT) against Docker and, when explicitly enabled,
#  the public internet. It then drives backend-specific round-trips
#  through the same lab proxies.
#
#  Every lab-backed backend with real proxy support gets an explicit
#  SOCKS4 / SOCKS5 / HTTP proxy testcase. Backends that cannot support
#  a proxy family at the library layer pin the warning/rejection path
#  instead of pretending direct traffic is acceptable.
# ════════════════════════════════════════════════════════════
...
def _recv_http_status(sock) -> bytes:
    """Read enough bytes to assert the proxied HTTP status line."""
    data = b""
...
def _http_minio_health_via_proxy(
    proxy_type: str,
    proxy_host: str,
    proxy_port: int,
) -> bytes:
    """Open a raw TCP tunnel via the requested proxy protocol and ask
    MinIO's unauthenticated health endpoint for a status line.

    This intentionally bypasses backend-specific SDKs so each proxy
    protocol gets a tiny, direct "does the tunnel actually work" test.
    Backend-level tests below then prove the protocol clients consume
    those proxy hooks correctly.
    """
    import socket as _socket

    from core.proxy import ProxyConfig, create_proxy_socket

    family = _socket.AF_INET if proxy_type == "socks4" else _socket.AF_UNSPEC
    sock = create_proxy_socket(
        ProxyConfig(proxy_type, proxy_host, proxy_port),
        S3_HOST,
        S3_PORT,
        timeout=10.0,
        family=family,
    ...
    try:
        request = (
            f"GET /minio/health/ready HTTP/1.1\r\n"
            f"Host: {S3_HOST}:{S3_PORT}\r\n"
            "Connection: close\r\n\r\n"
        ).encode("ascii")
        sock.sendall(request)
        return _recv_http_status(sock)
    finally:
        sock.close()


def _example_https_head_via_proxy(
    proxy_type: str,
    proxy_host: str,
    proxy_port: int,
) -> bytes:
    """Internet smoke: tunnel TLS to example.com through the lab proxy."""
    import socket as _socket
    import ssl as _ssl
    ...
    if proxy_type == "socks4":
        # Classic SOCKS4 has no hostname field. Resolve to IPv4 locally
        # and still use SNI/Host=example.com inside the TLS tunnel.
    ...
    try:
        ...
        with ctx.wrap_socket(sock, server_hostname=target_host) as tls:
            ...
            return _recv_http_status(tls)
    except Exception:
        sock.close()
        raise


def _internet_proxy_smokes_enabled() -> bool:
...
def test_proxy_protocol_socks4_raw_tcp_via_docker():
    if not wait_for_port(SOCKS4_PROXY_HOST, SOCKS4_PROXY_PORT, timeout=2):
        pytest.skip("SOCKS proxy container not reachable")
    if not wait_for_port(S3_HOST, S3_PORT, timeout=2):
        pytest.skip("MinIO container not reachable")
    with _ssrf_opt_in():
        status = _http_minio_health_via_proxy(
            "socks4",
            SOCKS4_PROXY_HOST,
            SOCKS4_PROXY_PORT,
        )
    assert status.startswith(b"HTTP/"), status
...
def test_proxy_protocol_http_connect_raw_tcp_via_docker():
    if not wait_for_port(HTTP_PROXY_HOST, HTTP_PROXY_PORT, timeout=2):
        pytest.skip("HTTP proxy container not reachable")
    if not wait_for_port(S3_HOST, S3_PORT, timeout=2):
        pytest.skip("MinIO container not reachable")
    with _ssrf_opt_in():
        status = _http_minio_health_via_proxy(
...
@pytest.mark.skipif(
    not _internet_proxy_smokes_enabled(),
    reason="set AXROSS_LIVE_INTERNET_PROXY_TESTS=1 to run internet proxy smokes",
)
@_label("Proxy 21d.0d SOCKS4 proxy protocol smoke via internet")
def test_proxy_protocol_socks4_raw_tcp_via_internet():
    with _ssrf_opt_in():
        status = _example_https_head_via_proxy(
            "socks4",
            SOCKS4_PROXY_HOST,
            SOCKS4_PROXY_PORT,
        )
...
@_label("Proxy 21d.0e SOCKS5 proxy protocol smoke via internet")
def test_proxy_protocol_socks5_raw_tcp_via_internet():
    with _ssrf_opt_in():
        status = _example_https_head_via_proxy(
            "socks5",
            SOCKS5_PROXY_HOST,
...
@_label("Proxy 21d.0f HTTP CONNECT proxy protocol smoke via internet")
def test_proxy_protocol_http_connect_raw_tcp_via_internet():
    with _ssrf_opt_in():
        status = _example_https_head_via_proxy(
            "http",
            HTTP_PROXY_HOST,
...
def test_telnet_via_http_connect_proxy():
    ...
    happy-path test instead of merely pinning the old ACL refusal.
    """
    with _ssrf_opt_in():
        s = _proxied_telnet_session(
            "http",
...
def _require_lab_endpoint(name: str, host: str, port: int) -> None:
    if not wait_for_port(host, port, timeout=2):
        pytest.skip(f"{name} container not reachable at {host}:{port}")


def _require_proxy_endpoint(proxy_type: str, host: str, port: int) -> None:
    if not wait_for_port(host, port, timeout=2):
        pytest.skip(f"{proxy_type} proxy container not reachable")
...
def test_proxy_matrix_webdav_round_trip(proxy_type, proxy_host, proxy_port):
    ...
    _require_lab_endpoint("WebDAV", WEBDAV_HOST, WEBDAV_PORT)
    path = f"/proxy_matrix_{proxy_type}.txt"
    with _ssrf_opt_in():
        s = _proxied_webdav_session(proxy_type, proxy_host, proxy_port)
        try:
            with s.open_write(path) as f:
                f.write(f"webdav {proxy_type}\n".encode("ascii"))
            with s.open_read(path) as f:
                assert f.read() == f"webdav {proxy_type}\n".encode("ascii")
        finally:
            try:
                s.remove(path)
            except Exception:
...
def test_proxy_matrix_ftp_round_trip(proxy_type, proxy_host, proxy_port):
    ...
    with _ssrf_opt_in():
        ...
        path = s.join(s.home(), f"proxy_matrix_{proxy_type}.txt")
        try:
            with s.open_write(path) as f:
                f.write(f"ftp {proxy_type}\n".encode("ascii"))
            with s.open_read(path) as f:
                assert f.read() == f"ftp {proxy_type}\n".encode("ascii")
        finally:
            try:
                s.remove(path)
            except Exception:
...
def test_proxy_matrix_ftps_read(proxy_type, proxy_host, proxy_port):
    ...
    with _ssrf_opt_in():
        ...
        try:
            ...
            with s.open_read(s.join(home, "data", "readme.txt")) as f:
                content = f.read()
            assert b"FTPS" in content or b"ftp" in content.lower(), content
        finally:
            s.disconnect()
...
def test_proxy_matrix_smb_round_trip(proxy_type, proxy_host, proxy_port):
    ...
    with _ssrf_opt_in():
        ...
        try:
            with s.open_write(path) as f:
                f.write(f"smb {proxy_type}\n".encode("ascii"))
            with s.open_read(path) as f:
                assert f.read() == f"smb {proxy_type}\n".encode("ascii")
        finally:
            try:
                s.remove(path)
            except Exception:
...
def test_proxy_matrix_imap_list(proxy_type, proxy_host, proxy_port):
    ...
    _require_proxy_endpoint(proxy_type, proxy_host, proxy_port)
    _require_lab_endpoint("IMAP", IMAP_HOST, IMAP_PORT)
    with _ssrf_opt_in():
        s = ImapSession(
            host=IMAP_HOST,
            port=IMAP_PORT,
        ...
        try:
            assert isinstance(s.list_dir("/"), list)
        finally:
            s.disconnect()
...
def test_proxy_matrix_pop3_list(proxy_type, proxy_host, proxy_port):
    ...
    _require_proxy_endpoint(proxy_type, proxy_host, proxy_port)
    _require_lab_endpoint("POP3", POP3_HOST, POP3_PORT_PLAIN)
    with _ssrf_opt_in():
        s = Pop3Session(
            host=POP3_HOST,
            port=POP3_PORT_PLAIN,
            username=POP3_USER,
            password=POP3_PASS,
            use_ssl=False,
            proxy_type=proxy_type,
...
def test_proxy_matrix_telnet_list(proxy_type, proxy_host, proxy_port):
    ...
    _require_lab_endpoint("Telnet", TELNET_HOST, TELNET_PORT)
    with _ssrf_opt_in():
        s = _proxied_telnet_session(proxy_type, proxy_host, proxy_port)
        try:
            assert isinstance(s.list_dir(s.home()), list)
        finally:
            s.disconnect()
...
def test_proxy_matrix_cisco_telnet_show(proxy_type, proxy_host, proxy_port):
    ...
    _require_proxy_endpoint(proxy_type, proxy_host, proxy_port)
    _require_lab_endpoint("Cisco fake", CISCO_HOST, CISCO_PORT)
    with _ssrf_opt_in():
...
def test_proxy_matrix_rsync_daemon_read(proxy_type, proxy_host, proxy_port):
    ...
    with _ssrf_opt_in():
        ...
        try:
            with s.open_read("/readme.txt") as f:
                assert b"rsync test file" in f.read()
        finally:
            s.disconnect()


@pytest.mark.parametrize("proxy_type,proxy_host,proxy_port", _PROXY_PROTOCOL_CASES)
def test_proxy_matrix_azure_blob_round_trip(proxy_type, proxy_host, proxy_port):
    from core.azure_client import AzureBlobSession

    _require_proxy_endpoint(proxy_type, proxy_host, proxy_port)
    container = _ensure_azurite_container()
    path = f"/proxy-matrix-{proxy_type}.txt"
    with _ssrf_opt_in():
        s = AzureBlobSession(
            connection_string=AZURITE_CONNECTION_STRING,
            container=AZURITE_CONTAINER,
        ...
        try:
            with s.open_write(path) as f:
                f.write(f"azure {proxy_type}\n".encode("ascii"))
            with s.open_read(path) as f:
                assert f.read() == f"azure {proxy_type}\n".encode("ascii")
        finally:
            try:
                s.remove(path)
            except Exception:
            ...
            try:
                ...
                try:
                    container.delete_blob(path.lstrip("/"))
                except Exception:
                    pass
...
def test_proxy_matrix_s3_socks_rejected(proxy_type, proxy_host, proxy_port):
    ...
    with _ssrf_opt_in():
        try:
            ...
            assert "socks" in str(exc).lower(), exc
            return
...
def test_proxy_matrix_s3_http_round_trip():
    ...
    _require_proxy_endpoint("http", HTTP_PROXY_HOST, HTTP_PROXY_PORT)
    _require_lab_endpoint("S3/MinIO", S3_HOST, S3_PORT)
    with _ssrf_opt_in():
        s = S3Session(
            bucket=S3_BUCKET,
        ...
        try:
            with s.open_write("/proxy_matrix_http.txt") as f:
                f.write(b"s3 http\n")
            with s.open_read("/proxy_matrix_http.txt") as f:
                assert f.read() == b"s3 http\n"
        finally:
            try:
                s.remove("/proxy_matrix_http.txt")
            except Exception:
                pass
            s.disconnect()
...
def test_s3_via_socks5_proxy_documents_limitation():
    ...
    with _ssrf_opt_in():
        try:
            ...
            assert "socks" in msg and "http" in msg, f"unexpected error shape: {exc}"
            return
        raise AssertionError(
...
def _tftp_session(filelist_enabled=False, filelist=None, max_size=16 * 1024 * 1024):
    from core.tftp_client import TFTPY_AVAILABLE, TftpSession
...
def test_iscsi_targets_discover_parses_iscsiadm_output(monkeypatch):
    ...
    monkeypatch.setattr(iscsi_client.shutil, "which", lambda name: "/usr/sbin/iscsiadm")
    monkeypatch.setattr(iscsi_client.subprocess, "run", lambda *a, **kw: _Done())
...
def test_iscsi_targets_discover_surfaces_iscsiadm_failure(monkeypatch):
    ...
    monkeypatch.setattr(iscsi_client.shutil, "which", lambda name: "/usr/sbin/iscsiadm")
    monkeypatch.setattr(iscsi_client.subprocess, "run", lambda *a, **kw: _Fail())
    try:
...
def test_iscsi_lun_list_filters_to_iscsi_transport_only(monkeypatch):
    ...
    monkeypatch.setattr(iscsi_client.shutil, "which", lambda name: "/usr/bin/lsblk")
    monkeypatch.setattr(iscsi_client.subprocess, "run", lambda *a, **kw: _Done())
...
class _FakeBlobClient:
    ...
    def create_snapshot(self, metadata=None):
        ...
        self.snapshots.append({"snapshot": snapshot_id, "metadata": metadata})
...
def test_azure_lease_duration_validation():
    ...
    for bad in (0, 5, 14, 61, 120):
        try:
            ...
            raise AssertionError(f"lease_acquire should refuse duration={bad}")
...
def test_azure_sas_url_signs_when_key_present():
    ...
    assert url.startswith("https://fakeaccount.blob.core.windows.net/fake/x.txt?")
    assert "sig=" in url
...
@pytest.mark.skipif(
    not _round2_cisco_reachable(), reason="cisco-fake container not reachable on 10.99.0.43:23"
)
def test_cisco_raw_show_passthrough():
    from core.telnet_cisco import CiscoTelnetSession

    s = CiscoTelnetSession(
        host=CISCO_HOST, port=CISCO_PORT, username=CISCO_USER, password=CISCO_PW, timeout=5.0
    )
    try:
        out = s.show("arp")
...
def _git_fs_session(tmp_path):
    ...
    return GitFsSession(path=str(repo_dir), author_name="Tester", author_email="t@axross.test")
...
@pytest.mark.skipif(
    not _tier2_webdav_up(), reason="webdav lab container (Date-header source) not reachable"
)
...
@pytest.mark.skipif(not _tier2_internet_reachable(), reason="no outbound HTTPS for RDAP whois")
...
@pytest.mark.skipif(
    not _tier2_ssh_alpha_reachable(), reason="ssh-alpha lab container not reachable"
)
...
def test_tier2_port_scan_concurrency_capped():
    ...
    for bad in (0, -1, 2000, 100000):
        try:
            ...
            raise AssertionError(f"port_scan should refuse concurrency={bad}")
...
@pytest.mark.skipif(not _telnet_up(), reason="Telnet lab container not reachable")
def test_telnet_send_raw_and_expect_round_trip():
    ...
    try:
        ...
        text, matched = s.expect(r"axross-telnet-marker-7331", timeout=5.0)
...
def test_slp_services_browse_dispatches_via_internal_helpers(monkeypatch):
    ...
    monkeypatch.setattr(s, "_fetch_types", lambda: ["service:printer:lpd", "service:ssh"])
    monkeypatch.setattr(
        s,
        "_fetch_urls",
        lambda t: {
    ...
    monkeypatch.setattr(
        ...
        "_fetch_attrs",
        lambda u: {
            "service:printer:lpd://10.0.0.1/queue": "(model=HP-LJ),(loc=floor-3)",
...
def test_slp_attributes_get_via_helper(monkeypatch):
    ...
    s = SlpSession.__new__(SlpSession)
    monkeypatch.setattr(s, "_fetch_attrs", lambda u: "(name=foo),(version=2.6)")
...
def test_pjl_info_category_allow_list_refuses_smuggling():
    ...
    for bad in ("STATUS\nFSDELETE 0:/etc/passwd", "ID\rEJECT", "BOGUS"):
        try:
            s.info(bad)
        except ValueError as e:
...
def test_pjl_info_id_round_trip():
    ...
    host, port, stop = _pjl_in_proc(
        {
            b"@PJL INFO ID": b'@PJL INFO ID\r\nID="axross-fake-printer"\r\n',
            b"@PJL INFO STATUS": b'@PJL INFO STATUS\r\nCODE=10001\r\nDISPLAY="READY"\r\n',
        }
    )
    try:
...
def test_nntp_group_arguments_refuse_crlf_smuggling():
    ...
    for verb, call in (
        ...
        ("article_headers", lambda: s.article_headers("alt.test\rA", 1)),
...
def test_nfs_exports_list_parses_showmount_output(monkeypatch):
    ...
    monkeypatch.setattr(nfs_client.shutil, "which", lambda name: "/usr/bin/showmount")
    monkeypatch.setattr(nfs_client.subprocess, "run", lambda *a, **kw: _Done())
...
def test_nfs_exports_list_surfaces_showmount_failure(monkeypatch):
    ...
    monkeypatch.setattr(nfs_client.shutil, "which", lambda name: "/usr/bin/showmount")
    monkeypatch.setattr(nfs_client.subprocess, "run", lambda *a, **kw: _Fail())
    try:
...
def test_gopher_recursive_fetch_walks_tree_with_caps():
    ...
    sub_menu = b"0b.txt\t/sub/b.txt\texample\t70\r\n0c.txt\t/sub/c.txt\texample\t70\r\n.\r\n"
    bodies = {
        "/a.txt": b"alpha-leaf",
    ...
    try:
        ...
        try:
            ...
            assert sorted(out.keys()) == sorted(["/a.txt", "/sub/b.txt", "/sub/c.txt"]), out
...
@pytest.mark.skipif(not _ftp_up(), reason="FTP lab container not reachable")
...
@pytest.mark.skipif(not _ldap_up(), reason="LDAP lab container not reachable")
def test_ldap_list_dir_navigates_tree():
    ...
    s = LdapFsSession(host=LDAP_HOST, port=LDAP_PORT, username=LDAP_BIND_DN, password=LDAP_BIND_PW)
    try:
        # Root → namingContexts.
        roots = s.list_dir("/")
...
def test_ldap_open_read_returns_ldif():
    ...
    s = LdapFsSession(host=LDAP_HOST, port=LDAP_PORT, username=LDAP_BIND_DN, password=LDAP_BIND_PW)
    try:
        body = s.open_read("/" + LDAP_BASE + "/ou=people/cn=alice").read().decode()
...
def test_ldap_path_dn_round_trip():
    ...
    assert p2d("/dc=example,dc=com/ou=people/cn=alice") == "cn=alice,ou=people,dc=example,dc=com"
...
def test_ldap_path_dn_refuses_rdn_injection():
    ...
    for hostile in (
        ...
        try:
            ...
            raise AssertionError(f"_path_to_dn should refuse injection in {hostile!r}")
    # RFC-4514 escaped commas MUST still be accepted in deeper segments.
    assert p2d("/dc=test,dc=com/cn=Doe\\, Jane") == "cn=Doe\\, Jane,dc=test,dc=com"
...
def test_snmp_get_timeout_when_host_silent():
    ...
    try:
        ...
        snmp_get("192.0.2.1", "1.3.6.1.2.1.1.1.0", community="public", timeout=0.5, retries=0)
    except OSError as e:
        # Either timeout or unreachable. Both surface as OSError.
        assert "SNMP" in str(e) or "timeout" in str(e).lower() or "unreachable" in str(e).lower(), (
            str(e)
        )
    else:
...
@pytest.mark.skipif(not _snmp_up(), reason="SNMP daemon not reachable on lab fixture")
def test_snmp_get_walk_set_round_trip():
    ...
    from core.snmp_helpers import snmp_get, snmp_set, snmp_walk

    sys_descr = snmp_get(
        SNMP_HOST, "1.3.6.1.2.1.1.1.0", community="public", port=SNMP_PORT, timeout=2.0
    )
    assert sys_descr.value, sys_descr

    walk = snmp_walk(
        SNMP_HOST, "1.3.6.1.2.1.1", community="public", port=SNMP_PORT, timeout=2.0, max_vars=20
    )
    ...
    after = snmp_get(
        SNMP_HOST, "1.3.6.1.2.1.1.6.0", community="public", port=SNMP_PORT, timeout=2.0
    )
...
def test_content_magic_type_known_formats():
    ...
    assert magic_type(b"\x00\x01\x02\x03random-junk") == "application/octet-stream"
...
def test_content_helpers_fallback_when_libs_missing(monkeypatch):
    ...
    class _Block:
        def find_spec(self, name, path=None, target=None):
            if (
                name in ("puremagic", "chardet")
                or name.startswith("puremagic.")
                or name.startswith("chardet.")
            ):
    ...
    saved = {
        ...
        if k in ("puremagic", "chardet") or k.startswith("puremagic.") or k.startswith("chardet.")
    }
    finder = _Block()
...
@pytest.mark.skipif(not _s3_up(), reason="S3 / MinIO lab container not reachable")
...
@pytest.mark.skipif(not _imap_up(), reason="imap-server lab container not reachable")
def test_imap_search_returns_uids():
    ...
    try:
        ...
        narrowed = s.search('SUBJECT "this-string-cannot-match-xyzzyx"', mailbox="INBOX")
        assert narrowed == []
    finally:
        s.disconnect()
...
@pytest.mark.skipif(not _pop3_up(), reason="pop3 lab container not reachable")
def test_pop3_stat_uidl_top():
    from core.pop3_client import Pop3Session

    s = Pop3Session(
        host=POP3_HOST, port=POP3_PORT_PLAIN, username=POP3_USER, password=POP3_PASS, use_ssl=False
    )
    try:
        ...
        assert top.startswith(
            (b"From:", b"Return-Path:", b"Received:", b"Subject:", b"Delivered-To:", b"To:")
        )
    finally:
        s.disconnect()
...
def test_db_sqlite_query_round_trip(tmp_path):
    ...
    try:
        ...
        s.query("CREATE TABLE users(id INTEGER PRIMARY KEY, name TEXT, age INT)")
...
@pytest.mark.skipif(not _ssh_alpha_up(), reason="ssh-alpha lab container not reachable")
...
def test_net_port_scan_concurrent():
    ...
    open_ports = port_scan("10.99.0.10", [21, 22, 23, 80, 443, 8080], timeout=0.5)
...
def test_net_dns_records_fallback_when_no_dnspython(monkeypatch):
    ...
    saved = {
        k: _sys.modules.pop(k) for k in list(_sys.modules) if k == "dns" or k.startswith("dns.")
    }
    monkeypatch.setattr(
        _sys,
        "path",
...
@pytest.mark.skipif(not _rsh_available(), reason="system rsh client not on PATH")
def test_rsh_live_round_trip():
    ...
    try:
        ...
        seed = s.open_read("~/data/seed.txt").read()
        assert b"hello from rshd" in seed, seed

        # Write + readback + checksum.
        with s.open_write("~/data/probe.txt") as fh:
            fh.write(b"axross-rsh-live")
        assert s.open_read("~/data/probe.txt").read() == b"axross-rsh-live"
        cs = s.checksum("~/data/probe.txt", "sha256")
        assert cs.startswith("sha256:") and len(cs) == len("sha256:") + 64

        # Subdir + copy + recursive remove.
        s.mkdir("~/data/sub")
        s.copy("~/data/probe.txt", "~/data/sub/copy.txt")
        items = sorted(i.name for i in s.list_dir("~/data/sub"))
        assert items == ["copy.txt"], items
        s.remove("~/data/sub", recursive=True)
        s.remove("~/data/probe.txt")

        # Final state matches the original seed-only listing.
        names = sorted(i.name for i in s.list_dir("~/data"))
    ...
    if service_ready.get("SMB"):
        all_tests.extend(
            [
                test_smb_connect,
                test_smb_read,
                test_smb_list_subdir,
                test_smb_write_cycle,
                test_smb_mkdir,
                test_smb_rename,
                test_smb_stat,
                test_smb_via_cm,
    ...
    if service_ready.get("WebDAV"):
        all_tests.extend(
            [
                test_webdav_connect,
                test_webdav_read,
                test_webdav_list_subdir,
                test_webdav_write_cycle,
                test_webdav_mkdir,
                test_webdav_rename,
                test_webdav_via_cm,
            ]
    ...
    if service_ready.get("Rsync"):
        all_tests.extend(
            [
                test_rsync_connect,
                test_rsync_read,
                test_rsync_list_subdir,
                test_rsync_write_cycle,
                test_rsync_via_cm,
            ]
        )
    else:
