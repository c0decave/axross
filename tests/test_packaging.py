...
# (binary-path, mtime-ns) -> full ``strings`` output, or ``None`` if
# ``strings`` is unavailable on this host. mtime is part of the key so
# a rebuild during the test session invalidates the cache automatically.
_BUNDLE_STRINGS_CACHE: dict[tuple[str, int], str | None] = {}


def _bundle_strings_blob(binary: Path) -> str | None:
    """Return the cached ``strings -n 4`` output for ``binary``.

    ``None`` means ``strings`` is not installed; the caller should
    fall back to chunked byte search. Subprocess timeout / non-zero
    exit also returns ``None`` so the caller's fallback runs.
    """
    key = (str(binary), binary.stat().st_mtime_ns)
    if key in _BUNDLE_STRINGS_CACHE:
        return _BUNDLE_STRINGS_CACHE[key]
    blob: str | None
    try:
        ...
        blob = result.stdout if result.returncode == 0 else None
    except FileNotFoundError:
        blob = None
    _BUNDLE_STRINGS_CACHE[key] = blob
    return blob
...
def _terminate_process_group(proc: subprocess.Popen) -> None:
    """Stop a bundled binary plus any PyInstaller child it spawned."""
    try:
...
class BundleStringsCacheTests(unittest.TestCase):
    """Pin the per-binary ``strings`` cache that ``_bundle_contains``
    depends on. If this regresses, ``PackagingImportPresenceTests``
    silently goes from ~4 s back to ~40 s — the timing alone won't
    fail the suite, but it eats the no-prior-regression budget on
    a busy box. Pin the contract explicitly.
    """

    def setUp(self) -> None:
        # Tests must not share cache state with each other or with
        # earlier production calls in the same process.
        self._saved_cache = dict(_BUNDLE_STRINGS_CACHE)
        _BUNDLE_STRINGS_CACHE.clear()

    def tearDown(self) -> None:
        _BUNDLE_STRINGS_CACHE.clear()
        _BUNDLE_STRINGS_CACHE.update(self._saved_cache)

    def _fake_binary(self, body: bytes = b"hello strings cache\n") -> Path:
    ...
    def test_happy_second_call_hits_cache_and_skips_strings(self) -> None:
        ...
        with unittest.mock.patch(
            ...
            second = _bundle_strings_blob(binary)
        self.assertIsNotNone(first)
        self.assertIn("abracadabra-needle-zzz", first or "")
        self.assertEqual(first, second)
        # Whole point of the cache: ``strings`` runs exactly once
        # even though the helper was called twice.
        self.assertEqual(spy.call_count, 1)

    def test_edge_mtime_change_invalidates_cache(self) -> None:
        ...
        # Bump the mtime AND change the content, then re-call.
        binary.write_bytes(b"second-payload-marker\n")
        # File systems can give the same mtime on rapid rewrites;
        # force a future mtime so the test is deterministic.
        future = binary.stat().st_mtime + 5
        os.utime(binary, (future, future))
        second = _bundle_strings_blob(binary)
        self.assertIn("second-payload-marker", second or "")
        self.assertNotIn("first-payload-marker", second or "")

    def test_sad_missing_strings_binary_returns_none(self) -> None:
        # Sad: on a system without ``strings`` installed, the helper
        # must return ``None`` so the caller's chunked-byte fallback
        # runs. We simulate by patching subprocess.run to raise
        # FileNotFoundError (the exact exception subprocess raises
        # when the program isn't on $PATH).
        binary = self._fake_binary()
        with unittest.mock.patch(
            "tests.test_packaging.subprocess.run",
            side_effect=FileNotFoundError("strings"),
        ):
            blob = _bundle_strings_blob(binary)
        self.assertIsNone(blob)
        # The None is also cached so we don't keep re-trying a
        # missing tool 30 times per test run.
        with unittest.mock.patch(
            "tests.test_packaging.subprocess.run",
            side_effect=AssertionError("should not be called on cache hit"),
        ):
            blob2 = _bundle_strings_blob(binary)
        self.assertIsNone(blob2)
    ...
    def call(self, method: str, params: dict | None = None, req_id: int = 1) -> dict:
        ...
        if not line:
            ...
            raise AssertionError(
                f"stdio MCP closed before responding to {method!r}. stderr: {stderr[:500]!r}"
            )
...
class PackagingSmokeTests(unittest.TestCase):
    ...
    def test_slim_help(self) -> None:
        ...
        self.assertEqual(
            result.returncode, 0, f"help exited {result.returncode}: {result.stderr!r}"
        )
...
class PackagingFlavourDeltaTests(unittest.TestCase):
    ...
    def _import_check(self, flavour: str, module: str) -> bool:
        ...
        """
        raise NotImplementedError("use test_flavour_sizes_differ instead")
...
class PackagingImportPresenceTests(unittest.TestCase):
    ...
    def _bundle_contains(self, flavour: str, needle: str) -> bool:
        ...
        # Each bundled binary is ~130 MB; running ``strings`` on it
        # takes ~1.2 s. The presence/exclusion test classes ask for
        # 30+ different needles against the same binary, so without
        # caching the suite eats ~40 s of pure redundant subprocess
        # work — enough to flip ``no-prior-regression`` from clean
        # to a timeout on a busy box. Cache the full
        # ``strings`` blob per (binary path, mtime) and let in-string
        # search handle the per-needle queries cheaply.
        blob = _bundle_strings_blob(binary)
        if blob is not None:
            return needle in blob
        # Fallback: ``strings`` unavailable AND we couldn't read
        # the binary; do a chunked grep on the raw bytes. Slow but
        # portable; only runs on exotic boxes where strings is
        # missing.
...
class PackagingHttpServerTests(unittest.TestCase):
    ...
    def test_http_health_endpoint_responds(self) -> None:
        ...
        proc = subprocess.Popen(
            [str(binary), "--mcp-server", "--mcp-http", f"127.0.0.1:{port}"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            start_new_session=True,
        )
        try:
            # PyInstaller one-file startup can be slow on loaded CI hosts.
