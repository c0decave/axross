...
MAX_STARTUP_SCRIPT_BYTES = 1024 * 1024
...
def _ensure_private_history_dir() -> None:
    HISTORY_PATH.parent.mkdir(parents=True, exist_ok=True)
    try:
        os.chmod(HISTORY_PATH.parent, 0o700)
    except OSError:
        log.debug("cannot chmod REPL history dir", exc_info=True)


def _read_text_limited(path: Path, *, max_bytes: int, tail: bool = False) -> str:
...
class ReplWidget(QPlainTextEdit):
    ...
    def __init__(self, parent: QWidget | None = None, startup_path: Path | None = None):
        ...
        # Lines submitted in THIS session only (cleared by .save). Used
        # for the .save slash-command so the user gets just the current
        # script, not the cumulative cross-session history.
    ...
    def _maybe_run_startup(self, startup_path: Path | None) -> None:
        ...
        if not path.exists():
            return
        try:
            src = _read_text_limited(path, max_bytes=MAX_STARTUP_SCRIPT_BYTES)
    ...
    def _submit_current(self) -> None:
        ...
        if line.strip():
            ...
            # Track this-session-only source for .save (slash-commands
            # are filtered out — they're not Python).
            if not line.lstrip().startswith("."):
                self._session_source.append(line)
...
REPL slash commands (typed at the prompt; not Python):
  ...
  .save <name>                save THIS session's submitted source as a script
...
"""
    ...
    def _maybe_dispatch_slash(self, text: str) -> bool:
        ...
        if cmd == "save":
            if not arg:
                ...
                return True
            # Save the source submitted in THIS session only (slash
            # commands are already excluded at submit time). This is
            # what users intuitively expect from .save — not the
            # cumulative cross-session history.
            if not self._session_source:
                self._append("nothing to save (no Python submitted yet this session)\n")
                return True
    ...
    def _load_history(self) -> None:
        ...
        try:
            ...
            return
        self._history = [line for line in text.splitlines() if line.strip()][-HISTORY_LIMIT:]
    ...
    def _compact_history_file(self) -> None:
        ...
        try:
            ...
            fd = os.open(
                tmp_path,
                os.O_WRONLY | os.O_CREAT | os.O_TRUNC | _nofollow_flag(),
                0o600,
            )
            try:
                os.write(fd, payload)
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:  # noqa: N802 — Qt
        ...
        if key == Qt.Key.Key_D and event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            ...
            # The dock can't actually close itself from here (the
            # MainWindow owns it), so we always re-prompt.
            if not self._current_input().strip():
                if self._buffer:
                    self._buffer = []
                    self._append("\n")
                else:
                    self._append("\n[REPL: Ctrl-D — close the Console dock to exit]\n")
                self._write_prompt(PROMPT)
            return
...
class _MarkdownTab(QWidget):
    ...
    def _render(self) -> None:
        try:
            ...
            self._viewer.setPlainText(f"(failed to render reference: {exc})")
