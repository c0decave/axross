...
class HexEditorReadLimitError(OSError):
    """Raised when a backend returns more data than the hex editor cap."""


def _read_capped(handle, cap: int = MAX_HEX_FILE_SIZE) -> bytes:
    data = handle.read(cap + 1)
    if isinstance(data, str):
        data = data.encode("utf-8", errors="replace")
    if len(data) > cap:
        raise HexEditorReadLimitError(f"file exceeds {cap} byte hex editor cap")
    return bytes(data)
...
class HexWidget(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        for line_idx in range(start_line, end_line):
            ...
            for i, byte in enumerate(line_bytes):
                ...
                is_selected = self._selection_start >= 0 and min(
                    self._selection_start, self._selection_end
                ) <= byte_offset <= max(self._selection_start, self._selection_end)
    ...
    def wheelEvent(self, event: QWheelEvent) -> None:
        ...
        new_offset = max(
            0, min(self._scroll_offset + lines, self._total_lines() - self._visible_lines())
        )
    ...
    def _load_file(self) -> None:
        try:
            ...
            with self._backend.open_read(self._file_path) as f:
                self._original_data = _read_capped(f)
        ...
        try:
            ...
            try:
                ...
                if current != self._original_data:
                    ...
                    if reply != QMessageBox.StandardButton.Yes:
                        return
            except HexEditorReadLimitError as e:
                QMessageBox.warning(
                    self,
                    "File Too Large",
                    f"Cannot safely check remote changes:\n{e}",
                )
                return
            except OSError:
                pass
            ...
            atomic_write(self._backend, self._file_path, content)
