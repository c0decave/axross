    ...
    for bm in mgr._bookmarks:  # noqa: SLF001 — read-only walk
        if bm.backend_name == "Local":
            if os.path.exists(bm.path):
                ...
                broken.append({"name": bm.name, "path": bm.path, "reason": "local path missing"})
            continue
        profile = profiles.get(bm.profile_name)
        if profile is None:
            skipped.append({"name": bm.name, "reason": "no matching saved profile"})
            continue
        try:
        ...
        try:
            if sess.exists(bm.path):
                ...
                broken.append({"name": bm.name, "path": bm.path, "reason": "remote path missing"})
        finally:
            try:
                sess.close()
