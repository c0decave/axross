...
def pack(
    src_backend, src_root: str, dst_backend, dst_path: str, passphrase: str, overwrite: bool = False
) -> int:
    ...
    if dst_backend.exists(dst_path) and not overwrite:
        raise FileExistsError(f"archive destination already exists: {dst_path}")
    buf = io.BytesIO()
...
def unpack(src_backend, src_path: str, dst_dir_local: str, passphrase: str) -> int:
