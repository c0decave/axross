...
def seal_stream(
    ...
    if dst_backend.exists(dst_path) and not overwrite:
        raise FileExistsError(f"sealed destination already exists: {dst_path}")
    with src_backend.open_read(src_path) as src_fh, dst_backend.open_write(dst_path) as dst_fh:
        ...
        encrypt_stream(src_fh, dst_fh, passphrase=passphrase, chunk_size=frame_size)
    return dst_backend.stat(dst_path).size


def unseal_stream(
    src_backend, src_path: str, dst_backend, dst_path: str, passphrase: str, overwrite: bool = False
) -> int:
    """Decrypt a sealed stream backend → backend. Returns the
    plaintext length on the destination."""
    if dst_backend.exists(dst_path) and not overwrite:
        raise FileExistsError(f"plaintext destination already exists: {dst_path}")
    with src_backend.open_read(src_path) as src_fh, dst_backend.open_write(dst_path) as dst_fh:
