...
The `core.cred_attack` module (exposed as `axross.bruteforce`,
`axross.spray`, `axross.enumerate_users`) is intrusive by definition —
the OpSec framing for it is in [docs/CRED_ATTACK.md](CRED_ATTACK.md),
not here. Two cross-cutting points worth knowing without leaving this
file:

* The whole surface refuses to run unless the caller passes
  `authorized=True`. There is no env-var override and no remembered
  consent.
* `AXROSS_SECURITY_MODE=paranoid` blocks credential testing entirely.
  If you've set paranoid mode for the rest of your work, switch it off
  *for the assessment box* before invoking these functions — don't try
  to bypass per-call.
